...
export function backendProviderConfig(config: AiConfig, mode: BackendGenerationMode = "image") {
    ...
    // System channel execution settings and secrets are authoritative on the server.
    if (requestConfig.channelId) return { channelId: requestConfig.channelId, model: requestConfig.model, ...generationOptions };
    return {
