...
// BeginTaskProviderSubmission fences a non-idempotent upstream submission before
// network I/O. An interrupted submission without an ID must not be sent again.
func (r *Repository) BeginTaskProviderSubmission(id string) error {
	result := r.db.Model(&model.Task{}).
		Where("id = ? AND COALESCE(provider_request_id, '') = '' AND COALESCE(poll_stage, '') = ''", id).
		Updates(map[string]any{"poll_stage": "submission_unknown", "updated_at": time.Now()})
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected != 1 {
