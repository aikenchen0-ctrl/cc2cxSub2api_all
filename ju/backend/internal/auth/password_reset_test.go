...
func TestPasswordResetChangesPasswordConsumesCodeAndRevokesSessions(t *testing.T) {
	...
	svc.SetMailSender(func(_ EmailSettingValue, recipient string, subject string, body string) error {
		if recipient != user.Email || subject != "智能剧场密码重置验证码" {
...
func newPasswordResetTestService(t *testing.T) (*Service, *gorm.DB) {
	...
	settingJSON, err := json.Marshal(EmailSettingValue{Enabled: true, Host: "smtp.example.com", Port: 587, Encryption: "starttls", FromEmail: "noreply@example.com", FromName: "智能剧场", RegistrationAllowedDomains: []string{"example.com"}})
	if err != nil {
		t.Fatal(err)
	}
