...
func (s *Service) shouldDeferVideoProviderTask(task model.Task, decryptedInput string, err error) bool {
	...
	if resolved.InterfaceType == sub2APIVideoProtocol {
		return task.StartedAt != nil && time.Now().Before(task.StartedAt.Add(sub2APIVideoRecoveryMaxAge))
	}
	return resolved.InterfaceType == string(model.ChannelInterfaceNewAPIChannel2) && !newAPIChannel2TaskSyncExpired(task, err, time.Now())
}
