...
func runProtocolAdapterTaskWithTiming(ctx context.Context, input canvasGenerationInput, adapter protocol.Adapter, timing protocolPollTiming) (map[string]interface{}, error) {
	...
	if taskID == "" {
		...
		if input.Config.InterfaceType == sub2APIVideoProtocol {
		...
		if input.Config.InterfaceType == sub2APIVideoProtocol && taskID != "" {
			if err := persistSub2APIVideoRequest(ctx, taskID); err != nil {
				return nil, err
			}
			ctx = withSub2APIVideoTaskID(ctx, taskID)
		}
	...
	for deadline := providerPollingDeadline(ctx); time.Now().Before(deadline); {
		...
		if err != nil {
			if input.Config.InterfaceType == sub2APIVideoProtocol && retryableSub2APIVideoError(err) {
		...
		if state.TaskID != "" {
			if input.Config.InterfaceType == sub2APIVideoProtocol && state.TaskID != taskID {
				return nil, errors.New("Sub2API returned a different video task ID")
			}
			taskID = state.TaskID
		}
		switch state.Status {
...
func queryProtocolAdapterVideoTask(ctx context.Context, input canvasGenerationInput, adapter protocol.Adapter, taskID string) (map[string]interface{}, string, error) {
	...
	if input.Config.InterfaceType == sub2APIVideoProtocol && state.TaskID != "" && state.TaskID != taskID {
		return nil, "", errors.New("Sub2API returned a different video task ID")
	}
...
func executeProtocolBinaryRequest(ctx context.Context, config providerConfig, spec protocol.RequestSpec) ([]byte, string, error) {
	if config.InterfaceType == sub2APIVideoProtocol {
		ctx = context.WithValue(ctx, sub2APIVideoRequestKey{}, true)
	}
...
func finishProtocolAdapterResult(ctx context.Context, input canvasGenerationInput, adapter protocol.Adapter, request protocol.GenerationRequest, taskID string, result *protocol.Result) (map[string]interface{}, error) {
	...
	if input.Config.InterfaceType == sub2APIVideoProtocol && !strings.HasPrefix(strings.ToLower(mimeType), "video/") {
		return nil, errors.New("Sub2API content endpoint did not return a video")
	}
	if mimeType == "" {
