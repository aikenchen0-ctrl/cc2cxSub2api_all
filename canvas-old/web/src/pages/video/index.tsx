...
const LEGACY_GENERATION_DURATION_THRESHOLD_MS = 5 * 60 * 1000;
...
export default function VideoPage() {
    ...
    const retryResult = () => {
        // Retry always creates a new upstream task. A previous async task may
        // have expired or lost its owner binding, so polling its ID can only
        // produce a permanent 404 loop.
        void generate();
    };
    ...
    const resumePendingLogs = (items: GenerationLog[]) => {
        const staleBefore = Date.now() - VIDEO_TIMEOUT_MS;
        for (const log of items) {
            if (log.status !== "pending" || !log.task) continue;
            if (log.createdAt > 0 && log.createdAt < staleBefore) {
                // Do not resurrect tasks beyond the client timeout. Their
                // server-side owner binding may already be gone, which would
                // otherwise create a repeated 404 loop after every page load.
                void logStore.setItem(log.id, serializeLog({
                    ...log,
                    status: "failed",
                    durationMs: Math.max(0, Date.now() - log.createdAt),
                    error: t("apiErrors.videoTaskQueryFailed"),
                }));
                continue;
            }
        ...
        try {
            for (let attempt = 0; attempt < VIDEO_MAX_ATTEMPTS; attempt += 1) {
                ...
                try {
                    ...
                    // A transient status/content request failure must not discard
                    // the upstream task; keep polling the saved task ID.
                    lastPollError = error instanceof Error ? error.message : t("workbench.generationFailed");
                    state = { status: "pending" };
                }
                if (state.status === "completed") {
                    const stored = await storeGeneratedVideo(state.result);
                    const generationDurationMs = Date.now() - log.createdAt;
                    const nextVideo: GeneratedVideo = {
                        id: nanoid(),
                        url: stored.url,
                        storageKey: stored.storageKey,
                        durationMs: stored.durationMs || configuredVideoDurationMs(log),
                    ...
                    await saveLog({ ...log, status: "success", durationMs: generationDurationMs, video: nextVideo, error: undefined });
                    message.success(t("videoWorkbench.generated"));
                    return;
                }
                if (state.status === "failed") throw new Error(state.error);
                if (attempt === VIDEO_MAX_ATTEMPTS - 1) throw new Error(lastPollError || t("videoWorkbench.timeout"));
            ...
            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2">
                ...
                <div className="grid justify-items-end gap-2">
                    ...
                    <Tag className="m-0 flex h-6 items-center rounded-md px-1.5 text-xs leading-none" color="green">
                        {formatDuration(log.video?.durationMs || log.durationMs)}
                    </Tag>
                </div>
            </div>
        </button>
...
async function normalizeLog(log: Partial<GenerationLog>): Promise<GenerationLog> {
    ...
    const rawVideo = log.video?.storageKey ? { ...log.video, url: await resolveMediaUrl(log.video.storageKey, log.video.url) } : log.video;
    // Older records stored generation wall-clock time in video.durationMs.
    // Replace only clearly impossible media durations; normal video metadata is preserved.
    const video = rawVideo && rawVideo.durationMs > LEGACY_GENERATION_DURATION_THRESHOLD_MS
        ? { ...rawVideo, durationMs: configuredVideoDurationMs({ ...log, config }) }
        : rawVideo;
...
function configuredVideoDurationMs(log: Pick<GenerationLog, "config" | "seconds"> | Partial<GenerationLog>) {
    const seconds = Number(log.config?.videoSeconds || log.seconds || 0);
