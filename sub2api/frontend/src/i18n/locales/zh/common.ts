export default {
  ...
  nav: {
    quickApps: '快捷应用',
    quickAppsKeyUnavailable: '无法获取超级 API Key，请重新登录后重试。',
    quickAppsSsoUnavailable: '无法打开快捷应用，请允许弹出窗口并确认登录状态；若仍失败，请联系管理员检查单点登录配置。',
    quickAppsSsoNeedLogin: '请先登录后再打开快捷应用。',
    quickAppsSsoPopupBlocked: '浏览器拦截了弹出窗口，请允许本站弹出窗口后重试。',
    quickAppsSsoNotConfigured: '单点登录未配置。请设置 SUB2API_SSO_SECRET（至少 32 位）以及目标应用的 SSO_CALLBACK_URL 后重启服务。',
    infiniteCanvas: 'AI生图生视频',
    smartShortDrama: 'AI导演台',
    superCanvas: '超级改图',
    aiProductImage: 'AI商品图',
    eternalPpt: 'PPT生成',
    aiCut: 'AI剪辑',
    screen2code: 'AI一键复刻网站',
    aiExcel: 'AI表格',
    artQr: '艺术二维码',
    yibiao: 'AI找标投标',
    ai3d: '图生3D',
  ...
  agentProvisioning: {
    title: 'AgentAPI 分站管理',
    description: '管理分站控制面记录并查看其上报的部署状态。',
    streamStatus: '实时更新连接状态',
    live: '实时同步中',
    reconnecting: '正在重连',
    controlPlaneNoticeTitle: '部署 worker 需在受控 edge 节点单独安装',
    controlPlaneNotice: '仓库已提供 worker 源码，可编排部署、DNS/TLS/readyz 检查与激活；创建记录不会自行启动 worker。worker 使用由主站 API allowlist 限权的共享 provisioning-only 凭据，但仍须严格限制其 Docker/Nginx 主机权限。AgentAPI 运行实例仍暂用全局主站管理员 Key；逐 Agent runtime 凭据尚未实现。托管实例使用同一个 agt_ ID 订阅权威状态；暂停/撤销会阻断请求，状态未知/过期也会 fail closed。未安装配置 worker 时需由运维人工部署并确认 readyz。',
    create: '新建分站',
    slug: '分站标识',
    domainSuffix: '平台域名将使用此标识并以 .cc2.cx 结尾。',
    displayName: '显示名称',
    ownerMainUserId: '主站管理员用户 ID',
    planId: '套餐 ID',
    planIdHint: '目前只校验格式，不查询套餐是否存在。',
    brandName: '品牌名称',
    logoUrl: 'Logo 地址（HTTPS）',
    submitCreate: '保存控制记录',
    search: '搜索',
    searchPlaceholder: '分站标识、域名、名称或主站用户 ID',
    status: '状态',
    allStatuses: '全部状态',
    currentStep: '上报步骤',
    domainStatus: '域名状态',
    ...
    empty: '没有找到分站记录。',
    loadFailed: '无法加载分站记录。',
    createFailed: '无法创建控制记录。',
    actionFailed: '无法更新控制记录。',
    confirmRevoke: '输入 {id} 以确认撤销。',
    confirmReadiness: '我已从预期域名确认 {domain} 的 AgentAPI /readyz 返回成功，且服务端凭据、Owner 和持久卷均已配置。',
    confirmationMismatch: '确认文本必须与 Agent ID 完全一致。',
    readinessConfirmationRequired: '激活前必须确认外部就绪检查已通过。',
    activateTitle: '确认分站已就绪并激活？',
    activateMessage: '激活后，使用相同 Agent ID 的托管实例将开放注册与业务请求。请先确认部署与域名配置完成。',
    retryTitle: '重新尝试部署？',
    retryMessage: '这会清除本次可重试失败并重新执行分站部署。请先修复“最近错误”指出的问题。',
    suspendTitle: '请求暂停分站？',
    suspendMessage: '这会将 {domain} 的主站状态切换为暂停，托管 AgentAPI 实例会拒绝新的业务请求。',
    resumeTitle: '请求恢复分站？',
    resumeMessage: '这会恢复 {domain} 的主站期望状态；托管实例在确认状态后恢复业务请求。',
    revokeTitle: '撤销分站记录？',
    revokeMessage: '这会将 {domain} 永久标记为已撤销，托管实例会拒绝业务请求。此操作不可恢复。',
    statuses: {
      pending: '待处理',
      provisioning: '部署中',
      active: '运行中',
