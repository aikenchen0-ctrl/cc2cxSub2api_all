...
export async function getSuper(): Promise<{ api_key: string; name: string; status: string }> {
  const { data } = await apiClient.get<{ api_key: string; name: string; status: string }>('/keys/super')
  return data
}
