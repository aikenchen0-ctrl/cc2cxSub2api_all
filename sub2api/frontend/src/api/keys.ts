...
export async function getSuper(): Promise<{ has_key: boolean; name: string; status: string }> {
  const { data } = await apiClient.get<{ has_key: boolean; name: string; status: string }>('/keys/super')
  return data
}
