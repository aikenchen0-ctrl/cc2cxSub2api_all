  ...
  return mount(HomeView, {
    global: {
      stubs: {
        ...
        LzParticleScene: { template: '<div data-testid="lz-particle-scene" />' },
...
describe('HomeView compact mode', () => {
  ...
  it.each([undefined, false])('selects the default home when compact mode is %s', (enabled) => {
    ...
    expect(wrapper.find('[data-testid="lz-particle-scene"]').exists()).toBe(true)
  })

  it('keeps the particle home start button in Chinese and links to downloads', () => {
    const wrapper = mountHome()
    const links = wrapper.findAllComponents(RouterLinkStub)

    expect(wrapper.get('.lz-start').text()).toBe('开始')
    expect(links.some((link) => link.props('to') === '/downloads')).toBe(true)
  })
