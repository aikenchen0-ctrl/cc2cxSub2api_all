      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group="{ row }">
            <span v-if="isSuperKey(row)" class="text-sm text-gray-400 dark:text-dark-500">
              {{ t('keys.noGroup') }}
            </span>
...
const handleDelete = async () => {
  if (!selectedKey.value || isSuperKey(selectedKey.value)) return
...
const executeCcsImport = (row: ApiKey, clientType: CcSwitchClientType) => {
  ...
  const providerName = (publicSettings.value?.site_name || '超参池X').trim() || '超参池X'
