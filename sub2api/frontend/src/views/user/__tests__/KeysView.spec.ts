...
describe('user KeysView column settings', () => {
  ...
  it('does not offer group selection for the Super Key', async () => {
    listKeys.mockResolvedValueOnce({
      items: [{ ...createApiKey(), name: 'Sub2API Super Key' }],
      total: 1,
      page: 1,
      page_size: 20,
      pages: 1,
    ...
    const wrapper = await mountView()
    const groupCell = wrapper.get('[data-test="key-group"]')

    expect(groupCell.text()).toBe('keys.noGroup')
