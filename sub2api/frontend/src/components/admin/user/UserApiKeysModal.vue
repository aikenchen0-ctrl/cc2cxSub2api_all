  ...
  <BaseDialog :show="show" :title="t('admin.users.userApiKeys')" width="wide" @close="handleClose">
    <div v-if="user" class="space-y-4">
      ...
      <div v-else ref="scrollContainerRef" class="max-h-96 space-y-3 overflow-y-auto" @scroll="closeGroupSelector">
        <div v-for="key in apiKeys" :key="key.id" data-test="api-key-row" class="rounded-xl border border-gray-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800">
          ...
          <div class="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
            <div v-if="!isSuperKey(key)" data-test="group-control" class="flex items-center gap-1">
...
const selectedKeyForGroup = computed(() => {
  ...
  const key = apiKeys.value.find((item) => item.id === groupSelectorKeyId.value) || null
  return key && !isSuperKey(key) ? key : null
})

const isSuperKey = (key: Pick<ApiKey, 'name'>) => key.name === 'Sub2API Super Key'
