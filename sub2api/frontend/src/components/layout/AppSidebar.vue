<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      >
        <img v-if="settingsLoaded" :src="siteLogo || '/logo.jpg'" alt="Logo" class="h-full w-full object-contain" />
      </router-link>
    ...
    <nav ref="sidebarNavRef" class="sidebar-nav scrollbar-hide">
      <!-- Shared quick applications -->
      <div class="sidebar-section">
        <div class="sidebar-section-title" :class="{ 'sidebar-section-title-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
          <span class="sidebar-section-title-text text-xs font-medium text-gray-500 dark:text-gray-400" :class="{ 'sidebar-section-title-text-collapsed': sidebarCollapsed }">
            {{ t('nav.quickApps') }}
          </span>
        </div>
        <a
          v-for="item in quickAppItems"
          :key="item.href"
          :href="item.href"
          target="_blank"
      ...
      <!-- Control panel navigation starts with the dashboard entry. -->
      <div class="sidebar-section-title" :class="{ 'sidebar-section-title-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
...
// The brand lockup always returns to the public home page, regardless of the
// current user's role or which console section is active.
const homePath = '/'
...
const CanvasIcon = {
  render: () =>
    h(
      ...
      { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' },
      [
        h('rect', { x: '3.75', y: '3.75', width: '16.5', height: '16.5', rx: '2.25' }),
        h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'm7.5 16.5 3-3 2.25 2.25 2.25-2.25 1.5 1.5M7.5 8.25h.008v.008H7.5V8.25z' })
      ]
    )
}

const ShortDramaIcon = {
  render: () =>
    h(
      'svg',
      { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' },
      [
        h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M3.75 7.5h16.5M3.75 7.5v10.125A2.625 2.625 0 006.375 20.25h11.25a2.625 2.625 0 002.625-2.625V7.5M3.75 7.5l1.5-3.75h3l1.5 3.75 1.5-3.75 1.5 3.75 1.5-3.75 1.5 3.75 1.5-3.75 1.5 3.75' }),
        h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'm10 12.75 4.5 2.25-4.5 2.25v-4.5z' })
      ]
    )
}
...
const QrCodeIcon = {
  render: () =>
    h(
      ...
      { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' },
      [
        h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M3.75 3.75h6v6h-6zM14.25 3.75h6v6h-6zM3.75 14.25h6v6h-6zM14.25 14.25h2.25v2.25h-2.25zM18 18h2.25v2.25H18zM18 14.25h2.25' }),
        h('path', { 'stroke-linecap': 'round', d: 'M12 3.75v2.25M12 18v2.25M3.75 12H6M18 12h2.25' })
      ]
    )
}
...
const PresentationIcon = {
  render: () =>
    h(
      ...
      { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '1.5' },
      [
        h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M3.75 4.5h16.5M12 4.5v15m-5.25-3.75L3.75 19.5m15-3.75 3.5 3.75M6.75 8.25h10.5v6.75H6.75z' })
      ]
    )
}
...
const quickAppItems = computed(() => {
  // Vite development must always stay on the local services, even when the
  // dev server is opened through a LAN IP or reverse proxy hostname.
  const isLocalHost = import.meta.env.DEV || ['localhost', '127.0.0.1', '0.0.0.0', '::1'].includes(window.location.hostname.toLowerCase())
  // Keep the deployed Canvas shortcut on HTTPS because the Super Key is
  // passed during bootstrap and must not traverse a plaintext connection.
  const canvasUrl = isLocalHost ? 'http://localhost:3522' : 'https://canvas.cc2.cx'
  const shortDramaUrl = import.meta.env.VITE_SHORT_DRAMA_SSO_URL?.trim() || `${buildApiUrl('/auth/integrations/ju/start')}?next=%2Fprojects`
  ...
  const apiBaseUrl = isLocalHost ? 'http://localhost:18080' : 'https://api.cc2.cx'
    const makeUrl = (origin: string, baseUrl = apiBaseUrl, includeApiKey = false) => {
      ...
      // Quick apps opened from a local Sub2API instance must call that same
      // instance so the generated Superkey is valid for the target gateway.
      url.searchParams.set('baseUrl', baseUrl)
      if (includeApiKey && superApiKey.value) {
        // Keep the bootstrap key out of HTTP request lines, access logs and
        // Referer headers. Canvas consumes and clears this fragment on load.
        url.searchParams.delete('baseUrl')
        url.hash = new URLSearchParams({ baseUrl, apiKey: superApiKey.value }).toString()
      }
      return url.toString()
    }
  return [
    {
      ...
      href: makeUrl(canvasUrl),
      icon: CanvasIcon,
      includeApiKey: true,
    },
    {
      label: t('nav.smartShortDrama'),
      href: shortDramaUrl,
      icon: ShortDramaIcon,
      includeApiKey: false,
      sso: true,
    },
    { label: t('nav.eternalPpt'), href: makeUrl(pptUrl), icon: PresentationIcon, includeApiKey: true },
    { label: t('nav.artQr'), href: makeUrl(qrcodeUrl), icon: QrCodeIcon, includeApiKey: true },
  ]
})
...
async function openQuickApp(url: string, includeApiKey = true, useSSO = false) {
  if (useSSO) {
    ...
    if (!popup) {
      window.alert(t('nav.quickAppsKeyUnavailable'))
      return
    }
    try {
      ...
      const body = await response.json() as { data?: { redirect_url?: string }; redirect_url?: string }
      const location = body.data?.redirect_url || body.redirect_url
      if (!location) {
        popup.close()
        window.alert(t('nav.quickAppsKeyUnavailable'))
        return
      ...
      console.warn('Failed to start Yingce SSO', error)
      popup.close()
      window.alert(t('nav.quickAppsKeyUnavailable'))
    }
    return
  ...
  const isCanvasTarget = target.hostname === 'canvas.cc2.cx' || (target.hostname === 'localhost' && target.port === '3522')
  const setBootstrapKey = (key: string) => {
    if (!isCanvasTarget) {
      target.searchParams.set('apiKey', key)
      return
    }
    const baseUrl = target.searchParams.get('baseUrl') || (target.hostname === 'localhost' ? 'http://localhost:18080' : 'https://api.cc2.cx')
  ...
  if (includeApiKey && !superApiKey.value) {
    try {
      const result = await keysAPI.getSuper()
      superApiKey.value = result.api_key
      setBootstrapKey(result.api_key)
    } catch (error) {
      console.warn('Failed to load super API key for quick app', error)
      window.alert(t('nav.quickAppsKeyUnavailable'))
      return
    }
  }
  if (includeApiKey && superApiKey.value) {
    setBootstrapKey(superApiKey.value)
  }
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/account-weights', label: t('admin.accountWeights.title'), icon: ChartIcon },
...
onMounted(() => {
  if (authStore.isAuthenticated) {
    void keysAPI.getSuper()
      .then((result) => {
        superApiKey.value = result.api_key
      })
      .catch((error) => {
        console.warn('Failed to load super API key for quick apps', error)
      })
  }
