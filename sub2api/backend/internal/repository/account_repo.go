...
func (r *accountRepository) ListSchedulableByPlatform(ctx context.Context, platform string) ([]service.Account, error) {
	...
	if platform == service.PlatformOpenAI || platform == service.PlatformGrok {
		preds = append(preds, dbaccount.StatusIn(service.StatusActive, service.StatusError))
	} else {
		preds = append(preds,
			dbaccount.StatusEQ(service.StatusActive),
			tempUnschedulablePredicate(),
...
func (r *accountRepository) ListSchedulableByGroupIDAndPlatform(ctx context.Context, groupID int64, platform string) ([]service.Account, error) {
	...
	if platform == service.PlatformOpenAI || platform == service.PlatformGrok {
		opts.status = ""
		opts.statuses = []string{service.StatusActive, service.StatusError}
		opts.ignoreTransientState = true
	}
	return r.queryAccountsByGroup(ctx, groupID, opts)
}
...
func (r *accountRepository) queryAccountsByGroup(ctx context.Context, groupID int64, opts accountGroupQueryOptions) ([]service.Account, error) {
	...
	if len(opts.statuses) > 0 {
		preds = append(preds, dbaccount.StatusIn(opts.statuses...))
	} else if opts.status != "" {
