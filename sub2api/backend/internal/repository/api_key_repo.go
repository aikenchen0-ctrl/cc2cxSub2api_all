...
// GetByUserIDAndName is used by the idempotent per-user super key bootstrap.
// The name is reserved by the service layer and the active-row filter keeps
// soft-deleted credentials from being reused.
func (r *apiKeyRepository) GetByUserIDAndName(ctx context.Context, userID int64, name string) (*service.APIKey, error) {
	m, err := r.activeQuery().
...
func (r *apiKeyRepository) Delete(ctx context.Context, id int64) error {
	// Super Key 禁止用户自行删除，由 APIKeyService.Delete 拦截。
	// 管理员删除用户时必须能级联 tombstone 该用户的 Super Key，因此仓库层不再拦截。
