...
func TestAnthropicEventToResponses_ThinkingAfterTextKeepsMessageOutput(t *testing.T) {
	...
	if completed == nil || completed.Response == nil || len(completed.Response.Output) != 2 {
		t.Fatalf("completed output = %+v, want message and reasoning", completed)
	}
	if completed.Response.Output[0].Type != "message" || len(completed.Response.Output[0].Content) != 1 || completed.Response.Output[0].Content[0].Text != "answer" {
		t.Fatalf("message output = %+v", completed.Response.Output[0])
	}
}
...
func TestAnthropicEventToResponses_MultipleTextBlocksAdvanceContentIndex(t *testing.T) {
	...
	for _, item := range []struct {
		...
		feed(&AnthropicStreamEvent{Type: "content_block_start", Index: &item.idx, ContentBlock: &AnthropicContentBlock{Type: "text"}})
		feed(&AnthropicStreamEvent{Type: "content_block_delta", Index: &item.idx, Delta: &AnthropicDelta{Type: "text_delta", Text: item.text}})
		feed(&AnthropicStreamEvent{Type: "content_block_stop", Index: &item.idx})
	}
	var added []int
	for _, evt := range events {
		if evt.Type == "response.content_part.added" {
			added = append(added, evt.ContentIndex)
		}
	}
	if len(added) != 2 || added[0] != 0 || added[1] != 1 {
		t.Fatalf("content indexes = %v, want [0 1]", added)
	}
}
