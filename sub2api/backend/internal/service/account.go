...
func (a *Account) IsSchedulable() bool {
	...
	return a.isWithinSchedulingWindows()
}

// IsOpenAIErrorChannelEligible 允许 OpenAI/Grok 组内 status=error 但仍启用的账号
// 参与 429/503 换号。这些账号被上次失败标成 error，但用户并未关闭调度。
func (a *Account) IsOpenAIErrorChannelEligible() bool {
	...
	// 不看 TempUnschedulableUntil / RateLimitResetAt：否则 429/503 会把同组其它号全部踢出池子。
	return true
}

func (a *Account) isWithinSchedulingWindows() bool {
	now := time.Now()
	if a.AutoPauseOnExpired && a.ExpiresAt != nil && !now.Before(*a.ExpiresAt) {
		return false
	}
...
func (a *Account) IsModelSupported(requestedModel string) bool {
	if isVividVideoAccount(a) {
		capability, ok := mediaadapter.Lookup(strings.TrimSpace(requestedModel))
...
// 429/503 由 failover 层直接换号，不再消耗这里的同账号重试预算。
