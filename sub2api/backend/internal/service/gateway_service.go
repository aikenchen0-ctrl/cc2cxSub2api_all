...
// ClearStickySession drops session_hash 粘性绑定，供 429/503 换号后让后续请求落到其它渠道。
...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	...
	for _, acc := range accounts {
		...
		if platform == PlatformOpenAI && acc.IsOpenAIPassthroughEnabled() && !IsAutoDLBaseURL(acc.GetOpenAIBaseURL()) {
			if s.modelsListCache != nil {
