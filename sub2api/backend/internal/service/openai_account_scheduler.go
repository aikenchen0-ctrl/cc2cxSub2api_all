	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			...
			if hasGroupMetadata && s.service != nil {
				groupCompatible = ctx.Value(ctxkey.SuperAPIKey) == true || s.service.openAIAccountMatchesSchedulingGroup(selection.Account, req.GroupID)
			}
			if !groupCompatible ||
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	// 粘性账号拿不到槽位时立刻逃逸到负载均衡，避免 120s 排队把 Codex 串行长任务拖死。
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !account.IsSchedulable() && !account.IsOpenAIErrorChannelEligible() {
...
func buildOpenAIAccountSchedulerScoreSnapshot(
	...
	for _, candidate := range candidates {
		...
		score := OpenAIAccountSchedulerScoreSnapshot{
			...
			Factors:               map[string]float64{"priority": priorityFactor, "load": loadFactor, "queue": queueFactor, "error_rate": errorFactor, "ttft": ttftFactor, "reset": resetFactor, "quota_headroom": quotaHeadroomFactor, "upstream_cost": upstreamCostFactor},
		}
