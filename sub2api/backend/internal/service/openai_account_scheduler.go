...
type OpenAIAccountScheduleRequest struct {
	...
	// ForceStickyAccount is used by async media status polls. The account that
	// created the task remains bound even if it is temporarily unschedulable.
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if stickyAccountNeedsEviction(account, req) || account.Platform != NormalizeOpenAICompatiblePlatform(req.Platform) || !account.IsOpenAICompatible() {
		clearBinding()
		return nil, false, nil
	}
...
func stickyAccountNeedsEviction(account *Account, req OpenAIAccountScheduleRequest) bool {
	if account == nil {
		return true
	}
	if req.ForceStickyAccount {
		return false
	}
	return shouldClearStickySession(account, req.RequestedModel) || !account.IsSchedulable()
}
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		if stickyAccountNeedsEviction(account, req) {
