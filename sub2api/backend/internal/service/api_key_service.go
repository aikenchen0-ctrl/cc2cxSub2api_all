...
var (
	...
	ErrSuperAPIKeyProtected = infraerrors.Forbidden("SUPER_API_KEY_PROTECTED", "the Sub2API Super Key cannot be deleted")
...
type superAPIKeyFinder interface {
...
// GetOrCreateSuperAPIKey returns the user's single compatibility key. It is
// intentionally idempotent so the sidebar can request it whenever an app is
// opened without creating additional credentials.
func (s *APIKeyService) GetOrCreateSuperAPIKey(ctx context.Context, userID int64) (*APIKey, error) {
	if finder, ok := s.apiKeyRepo.(superAPIKeyFinder); ok {
		if key, err := finder.GetByUserIDAndName(ctx, userID, SuperAPIKeyName); err == nil && key != nil && key.IsSuper() {
			s.compileAPIKeyIPRules(key)
			return key, nil
		} else if err != nil && !errors.Is(err, ErrAPIKeyNotFound) {
			return nil, fmt.Errorf("find super api key: %w", err)
		}
	}
	key, err := s.GenerateKey()
	if err != nil {
		return nil, fmt.Errorf("generate super api key: %w", err)
	}
	key = SuperAPIKeyPrefix + strings.TrimPrefix(key, s.cfg.Default.APIKeyPrefix)
	created := &APIKey{UserID: userID, Key: key, Name: SuperAPIKeyName, Status: StatusActive}
	err = s.apiKeyRepo.Create(ctx, created)
	if err != nil {
		if finder, ok := s.apiKeyRepo.(superAPIKeyFinder); ok {
			if existing, findErr := finder.GetByUserIDAndName(ctx, userID, SuperAPIKeyName); findErr == nil && existing != nil {
				return existing, nil
			}
		}
		return nil, fmt.Errorf("create super api key: %w", err)
	}
...
// ResolveSuperAPIKeyGroup selects the target upstream group for a super key.
// Super keys may target any active group, independent of the user's bindings.
func (s *APIKeyService) ResolveSuperAPIKeyGroup(ctx context.Context, user *User, requestedID *int64) (*Group, error) {
	if user == nil || s.groupRepo == nil {
		return nil, ErrGroupNotAllowed
	}
	if requestedID != nil {
		group, err := s.groupRepo.GetByID(ctx, *requestedID)
		if err != nil {
			return nil, err
		}
	...
	// Prefer an image-capable group for the bundled applications. Super Keys are
	// intentionally cross-group credentials, and image generation must not get
	// pinned to an unrelated text-only group merely because it sorts first.
	for i := range groups {
		if !groups[i].AllowImageGeneration {
			continue
		}
		if superKeyCanUseGroup(&groups[i]) {
			return &groups[i], nil
		}
	}
	for i := range groups {
...
func (s *APIKeyService) ValidateCustomKey(key string) error {
	...
	if strings.HasPrefix(key, SuperAPIKeyPrefix) {
...
func (s *APIKeyService) Delete(ctx context.Context, id int64, userID int64) error {
	...
	if strings.HasPrefix(key, SuperAPIKeyPrefix) {
		return ErrSuperAPIKeyProtected
	}
