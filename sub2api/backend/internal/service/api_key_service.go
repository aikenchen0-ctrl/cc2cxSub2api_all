...
// SatelliteUserKey loads the user's Super Key for a satellite on-behalf-of
// call. The Super Key itself is never accepted as an inbound API credential.
func (s *APIKeyService) SatelliteUserKey(ctx context.Context, userID int64) (*APIKey, error) {
	if userID <= 0 {
		return nil, ErrAPIKeyNotFound
	}
	key, err := s.GetOrCreateSuperAPIKey(ctx, userID)
	if err != nil {
		return nil, err
	}
	user, err := s.userRepo.GetByID(ctx, userID)
	if err != nil {
		return nil, fmt.Errorf("get satellite user: %w", err)
	}
	key.User = user
	s.compileAPIKeyIPRules(key)
	return key, nil
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if apiKey.IsSuper() {
		return nil, ErrSuperAPIKeyProtected
	}
