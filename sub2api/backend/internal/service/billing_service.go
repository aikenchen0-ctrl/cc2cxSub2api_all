...
func (s *BillingService) getVideoUnitPrice(model string, resolution string, groupConfig *VideoPriceConfig) float64 {
	...
	if groupConfig != nil {
		...
		case VideoBillingResolution736P:
			// AutoDL b99 workflows use a 736p tier. There is no legacy flat
			// group column for it, so use the configured HD price unless the
			// per-model map above provided an explicit 736p price.
			if groupConfig.Price720P != nil {
				return *groupConfig.Price720P
			}
...
func getDefaultGrokImagineVideoPrice(model string, resolution string) (float64, bool) {
	...
	case strings.HasPrefix(model, "grok-imagine-video-1.5"):
		...
		case VideoBillingResolution720P, VideoBillingResolution736P:
	...
	case strings.HasPrefix(model, "grok-imagine-video"):
		...
		case VideoBillingResolution720P, VideoBillingResolution736P, VideoBillingResolution1080P:
