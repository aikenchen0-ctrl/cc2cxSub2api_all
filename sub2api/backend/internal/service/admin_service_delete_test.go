...
func TestAdminService_DeleteUser_DeletesOwnedSuperKey(t *testing.T) {
	superKey := SuperAPIKeyPrefix + "user-super"
	repo := &userRepoStub{user: &User{ID: 7, Role: RoleUser}}
	apiKeyRepo := &apiKeyRepoStub{
		allowListByUserID: true,
		listByUserIDKeys: []APIKey{
			{ID: 11, UserID: 7, Key: "sk-user-1", Name: "personal"},
			{ID: 12, UserID: 7, Key: superKey, Name: SuperAPIKeyName},
			{ID: 13, UserID: 7, Key: "sk-user-2", Name: "ci"},
		},
	}
	invalidator := &authCacheInvalidatorStub{}
	svc := &adminServiceImpl{
	...
	require.NoError(t, err)
	require.Equal(t, []int64{7}, repo.deletedIDs)
	require.Equal(t, []int64{11, 12, 13}, apiKeyRepo.deletedIDs)
	require.ElementsMatch(t, []string{"sk-user-1", superKey, "sk-user-2"}, invalidator.keys)
	require.Equal(t, []int64{7}, invalidator.userIDs)
}

func TestAdminService_DeleteUser_DeletesWhenOnlySuperKeyExists(t *testing.T) {
	superKey := SuperAPIKeyPrefix + "only"
	repo := &userRepoStub{user: &User{ID: 8, Role: RoleUser}}
	apiKeyRepo := &apiKeyRepoStub{
		allowListByUserID: true,
		listByUserIDKeys: []APIKey{
			{ID: 21, UserID: 8, Key: superKey, Name: SuperAPIKeyName},
		},
	}
	svc := &adminServiceImpl{
		userRepo:   repo,
	...
	require.Equal(t, []int64{21}, apiKeyRepo.deletedIDs)
}
