...
func TestApiKeyService_Delete_RejectsSuperKey(t *testing.T) {
	repo := &apiKeyRepoStub{
		apiKey: &APIKey{ID: 21, UserID: 8, Key: SuperAPIKeyPrefix + "user", Name: SuperAPIKeyName},
	}
	svc := &APIKeyService{apiKeyRepo: repo}

	err := svc.Delete(context.Background(), 21, 8)
	require.ErrorIs(t, err, ErrSuperAPIKeyProtected)
	require.Empty(t, repo.deletedIDs)
}
