...
func TestLookupVideoBillingResolutionAcceptsAutoDL736p(t *testing.T) {
	t.Parallel()
	got, ok := LookupVideoBillingResolution("736p")
	require.True(t, ok)
	require.Equal(t, VideoBillingResolution736P, got)
	normalized := NormalizeVideoModelPrices(map[string]map[string]float64{
		"autodl-model": {"736p": 0.201},
	})
	price := LookupVideoModelPrice(normalized, "autodl-model", "736p")
	require.NotNil(t, price)
	require.InDelta(t, 0.201, *price, 1e-9)
}
