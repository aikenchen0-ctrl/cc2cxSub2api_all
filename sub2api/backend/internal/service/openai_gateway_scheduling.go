...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if account == nil || (ctx.Value(ctxkey.SuperAPIKey) != true && !s.openAIAccountMatchesSchedulingGroup(account, groupID)) {
		_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
		return nil
	}
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if err == nil {
				...
				if !clearSticky && isOpenAICompatibleAccountEligibleForRequest(ctx, account, platform, requestedModel, false, requiredCapability) {
					...
					if account == nil {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
					} else if ctx.Value(ctxkey.SuperAPIKey) != true && !s.openAIAccountMatchesSchedulingGroup(account, groupID) {
						_ = s.deleteStickySessionAccountID(ctx, groupID, sessionHash)
...
func (s *OpenAIGatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string) ([]Account, error) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
	// Super API keys are explicitly cross-group. Bypass group-scoped snapshots
	// and load the complete platform pool so grouped accounts remain eligible.
	if ctx.Value(ctxkey.SuperAPIKey) == true {
		accounts, err := s.accountRepo.ListSchedulableByPlatform(ctx, platform)
		if err != nil {
			return nil, fmt.Errorf("query cross-group accounts failed: %w", err)
		}
	...
	if s.schedulerSnapshot != nil {
		...
		accounts = s.mergeOpenAIErrorChannelAccounts(ctx, accounts, groupID, platform)
...
func (s *OpenAIGatewayService) mergeOpenAIErrorChannelAccounts(ctx context.Context, accounts []Account, groupID *int64, platform string) []Account {
	...
	fresh, err := s.accountRepo.ListSchedulableByGroupIDAndPlatform(ctx, *groupID, platform)
	if err != nil || len(fresh) == 0 {
		return accounts
	}
	return mergeOpenAIAccountsByID(accounts, fresh)
}

func mergeOpenAIAccountsByID(base, extra []Account) []Account {
	if len(extra) == 0 {
		return base
	}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	// Super API keys intentionally span groups. Their initial account query is
	// cross-group, so the nil group ID must not make the DB recheck require an
	// ungrouped account.
	if ctx.Value(ctxkey.SuperAPIKey) != true && !s.openAIAccountMatchesSchedulingGroup(latest, groupID) {
		return nil
	}
