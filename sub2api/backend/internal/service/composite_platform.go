...
func DetectModelPlatform(model string) (string, bool) {
	...
	case strings.HasPrefix(normalized, "kling-"),
		...
		strings.HasPrefix(normalized, "seedance2"),
		strings.HasPrefix(normalized, "sd-2.5-"),
		strings.HasPrefix(normalized, "minimax-"),
		strings.HasPrefix(normalized, "minimax_"),
		strings.HasPrefix(normalized, "veo-"):
		// These models are exposed by compatible relays (for example
		// 计算万物) and use the OpenAI-compatible media contract.
		return PlatformOpenAI, true
	case normalized == "grok-imagine-video-1.5":
		return PlatformOpenAI, true
