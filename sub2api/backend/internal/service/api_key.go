...
// SuperAPIKeyName and SuperAPIKeyPrefix identify the per-user compatibility
// key used by the bundled applications. The prefix is reserved and rejected
// for user-supplied custom keys.
const (
	SuperAPIKeyName   = "Sub2API Super Key"
	SuperAPIKeyPrefix = "sk-super-"
)
...
func (k *APIKey) IsSuper() bool {
	return k != nil && strings.HasPrefix(k.Key, SuperAPIKeyPrefix) && k.Name == SuperAPIKeyName
}

// IsSuperAPIKeyCredential reports whether a presented secret is a Super Key.
// Super Keys are not API keys: /v1 must never accept them as Bearer/x-api-key.
func IsSuperAPIKeyCredential(key string) bool {
	return strings.HasPrefix(strings.TrimSpace(key), SuperAPIKeyPrefix)
}
