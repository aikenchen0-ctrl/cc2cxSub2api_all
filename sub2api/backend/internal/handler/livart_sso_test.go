...
func TestLivartSSOStartRejectsUnauthenticated(t *testing.T) {
	gin.SetMode(gin.TestMode)
	t.Setenv("SUB2API_SSO_SECRET", strings.Repeat("s", 32))
	t.Setenv("LIVART_SSO_CALLBACK_URL", "http://localhost:8080/api/auth/sso/callback")

	handler := &AuthHandler{userService: service.NewUserService(&userHandlerRepoStub{
		user: &service.User{ID: 7, Email: "user@example.com", Username: "canvas-user"},
	}, nil, nil, nil)}

	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/auth/integrations/livart/start", nil)

	handler.LivartSSOStart(c)
...
func TestLivartSSOStartIssuesCallbackTicketWithoutSecrets(t *testing.T) {
	gin.SetMode(gin.TestMode)
	secret := strings.Repeat("livart-sso-secret-", 2)
	require.GreaterOrEqual(t, len(secret), 32)
	t.Setenv("SUB2API_SSO_SECRET", secret)
	t.Setenv("LIVART_SSO_CALLBACK_URL", "http://localhost:8080/api/auth/sso/callback")

	handler := &AuthHandler{userService: service.NewUserService(&userHandlerRepoStub{
		user: &service.User{ID: 7, Email: "user@example.com", Username: "canvas-user", AvatarURL: "https://cdn.example/a.png"},
	}, nil, nil, nil)}

	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/auth/integrations/livart/start?next=%2F", nil)
	...
	require.Equal(t, "localhost:8080", parsed.Host)
...
func TestLivartSSOStartRejectsExternalNext(t *testing.T) {
	gin.SetMode(gin.TestMode)
	secret := strings.Repeat("s", 32)
	t.Setenv("SUB2API_SSO_SECRET", secret)
	t.Setenv("LIVART_SSO_CALLBACK_URL", "https://livart.example/api/auth/sso/callback")

	handler := &AuthHandler{userService: service.NewUserService(&userHandlerRepoStub{
		user: &service.User{ID: 9, Email: "next@example.com", Username: "next-user"},
	}, nil, nil, nil)}
	...
	for _, next := range []string{
		...
		recorder := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(recorder)
		c.Request = httptest.NewRequest(http.MethodGet, "/api/v1/auth/integrations/livart/start?next="+url.QueryEscape(next), nil)
		...
		parsed, err := url.Parse(resp.Data.RedirectURL)
		require.NoError(t, err)
		require.Equal(t, "livart.example", parsed.Host)
		payload := decodeLivartSSOTicket(t, parsed.Query().Get("ticket"), secret)
		require.Equal(t, "/", payload.Next, "next=%s", next)
		require.NotContains(t, payload.Next, "evil")
	}
}

func decodeLivartSSOTicket(t *testing.T, raw, secret string) juSSOTicket {
	t.Helper()
