...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	if isGrokVideoCreateEndpoint(endpoint) && strings.TrimSpace(requestInfo.ResolutionRaw) != "" {
		if _, ok := service.LookupVideoBillingResolution(requestInfo.ResolutionRaw); !ok {
			h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "unsupported video resolution")
			return
		}
	}
	...
	if endpoint.IsGenerationRequest() {
		if !apiKey.IsSuper() && !service.GroupAllowsImageGeneration(apiKey.Group) {
			h.errorResponse(c, http.StatusForbidden, "permission_error", service.ImageGenerationPermissionMessage())
			return
		}
		if service.ShouldRunMediaModeration(apiKey) {
		...
		if service.ShouldEnforceMediaConcurrency(apiKey) {
	...
	preferAutoDLVideo := false
	autoDLVideoAttempted := false
	preferVividVideo := false
	vividVideoAttempted := false
	if endpoint == service.GrokMediaEndpointVideosGenerations || endpoint.IsVideoLookupRequest() {
		modelName := strings.ToLower(strings.TrimSpace(routingModel))
		if endpoint.IsVideoLookupRequest() {
			// The task binding identifies the provider; allow either Grok or
			// OpenAI-compatible accounts during status/content lookup.
			selectionPlatform = ""
			requiredCapability = ""
			selectionModel = ""
		} else if service.IsOpenAICompatibleVideoModel(requestModel) || !strings.Contains(modelName, "grok") {
			// NewToken/VEO/MiniMax/video-* use the generic OpenAI-compatible adapter.
			selectionPlatform = service.PlatformOpenAI
			requiredCapability = ""
			// Keep the requested model for forwarding, but constrain scheduling to
			// the canonical AutoDL workflow when one exists. This prevents a
			// generic OpenAI account from being selected for an AutoDL-only model.
			selectionModel = requestModel
			if service.ShouldPreferAutoDLVideoModel(requestModel) {
				// AutoDL passthrough accounts intentionally have an empty model
				// mapping. Leave the scheduler model unconstrained and apply the
				// provider preference below, otherwise its cache may reject the
				// account before the AutoDL adapter can validate the workflow.
				selectionModel = ""
				preferAutoDLVideo = true
			}
			if requestModel != "" && !service.ShouldPreferAutoDLVideoModel(requestModel) {
				preferVividVideo = true
			}
		}
	}
	...
	for {
		...
		if err != nil {
			...
			// Vivid is a preference, not a hard requirement. If the first pass
			// exhausted non-Vivid candidates while looking for a Vivid account,
			// retry the original candidate pool so another compatible provider can
			// serve the request.
			if endpoint.IsGenerationRequest() && preferVividVideo && !vividVideoAttempted {
				failedAccountIDs = make(map[int64]struct{})
				preferVividVideo = false
				continue
			}
		...
		if endpoint.IsGenerationRequest() && selectionPlatform == service.PlatformOpenAI && preferAutoDLVideo && !autoDLVideoAttempted && !service.IsAutoDLBaseURL(account.GetOpenAIBaseURL()) {
			failedAccountIDs[account.ID] = struct{}{}
			continue
		}
		if endpoint.IsGenerationRequest() && selectionPlatform == service.PlatformOpenAI && preferVividVideo && !vividVideoAttempted && !service.IsVividVideoAccount(account) {
			failedAccountIDs[account.ID] = struct{}{}
			continue
		}
		if service.IsAutoDLBaseURL(account.GetOpenAIBaseURL()) {
			autoDLVideoAttempted = true
		}
		if service.IsVividVideoAccount(account) {
			vividVideoAttempted = true
		}
		if endpoint.IsGenerationRequest() && service.IsAutoDLBaseURL(account.GetOpenAIBaseURL()) && !service.AutoDLVideoModelSupported(account, requestModel) {
			// AutoDL exposes one workflow endpoint per model. Do not send an
			// unknown alias there; exclude the account and let the scheduler try
			// another OpenAI-compatible video provider.
			failedAccountIDs[account.ID] = struct{}{}
			if switchCount >= maxAccountSwitches {
				h.errorResponse(c, http.StatusServiceUnavailable, "video_model_unavailable", "No video account supports model: "+requestModel)
				return
			}
			switchCount++
			continue
		}
		if endpoint.IsGenerationRequest() && selectionPlatform == service.PlatformOpenAI && !service.OpenAICompatibleVideoModelSupported(account, requestModel) {
			failedAccountIDs[account.ID] = struct{}{}
			if switchCount >= maxAccountSwitches {
				h.errorResponse(c, http.StatusServiceUnavailable, "video_model_unavailable", "No video account supports model: "+requestModel)
				return
		...
		if endpoint.IsGenerationRequest() && selectionPlatform == service.PlatformGrok {
