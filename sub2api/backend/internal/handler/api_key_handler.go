...
// GetSuperAPIKey returns (and lazily creates) the authenticated user's
// cross-group compatibility key. The full credential is returned only from
// this authenticated endpoint so the bundled applications can bootstrap.
// GET /api/v1/keys/super
func (h *APIKeyHandler) GetSuperAPIKey(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	}
	key, err := h.apiKeyService.GetOrCreateSuperAPIKey(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
