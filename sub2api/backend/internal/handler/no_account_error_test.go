...
func TestClassifySelectionFailureError_ModelNotFoundIsAuthoritative(t *testing.T) {
	fallback := noAccountErrorClassification{Status: http.StatusNotFound, ErrType: "model_not_found", Message: "unsupported", ModelNotFound: true}
	require.Equal(t, fallback, classifySelectionFailureError(fmt.Errorf("model_rate_limited=3"), fallback))
}
