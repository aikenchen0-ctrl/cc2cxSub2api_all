...
func APIKeyAuthWithSubscriptionGoogle(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if service.IsSuperAPIKeyCredential(apiKeyString) {
			abortWithGoogleError(c, http.StatusForbidden, "Super Key cannot be used as an API key")
			return
		}
		...
		if satelliteHandled || apiKey.IsSuper() {
			...
			if groupValue != "" {
				parsed, parseErr := strconv.ParseInt(groupValue, 10, 64)
				if parseErr != nil || parsed <= 0 {
					abortWithGoogleError(c, http.StatusBadRequest, "Invalid target group id")
					return
				}
			...
			if requestedGroupID != nil {
				...
				if groupErr != nil {
					abortWithGoogleError(c, http.StatusForbidden, "No available target group for this API key")
					return
				}
				clone := *apiKey
				clone.Group = group
