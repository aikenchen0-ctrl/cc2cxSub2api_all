...
func TestRequireGroupAssignmentAllowsUnboundSuperKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
	settingService := service.NewSettingService(fakeSettingRepo{values: map[string]string{
		service.SettingKeyAllowUngroupedKeyScheduling: "false",
	}}, &config.Config{})
	apiKey := &service.APIKey{ID: 101, Key: service.SuperAPIKeyPrefix + "test-user", Name: service.SuperAPIKeyName, Status: service.StatusActive}
...
func TestValidateAPIKeyGroupAllowedBypassesBindingForSuperKey(t *testing.T) {
	groupID := int64(99)
	key := &service.APIKey{
		Key:     service.SuperAPIKeyPrefix + "test",
	...
	if !validateAPIKeyGroupAllowed(key) {
		t.Fatal("super key must bypass user group binding checks")
	}
}
