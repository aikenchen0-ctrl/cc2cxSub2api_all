...
func TestGroupModelAllowlistSuperKeyBypassesEnabledList(t *testing.T) {
	super := allowlistAPIKey(true, "claude-sonnet-4.5")
	super.Key = service.SuperAPIKeyPrefix + "test"
	super.Name = service.SuperAPIKeyName
	router, calls := newGroupModelAllowlistTestRouter(super, "/v1")
	w := doJSON(t, router, http.MethodPost, "/v1/messages", `{"model":"claude-opus-4.6"}`)
	if w.Code != http.StatusOK {
		t.Fatalf("super key must bypass allowlist, got %d: %s", w.Code, w.Body.String())
	}
	if len(*calls) != 1 {
		t.Fatalf("expected handler to run, got %v", *calls)
	}
}
