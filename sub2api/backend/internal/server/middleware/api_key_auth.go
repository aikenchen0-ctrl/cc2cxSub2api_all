...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if service.ShouldEnforceAPIKeyIPRestrictions(apiKey) && (len(apiKey.IPWhitelist) > 0 || len(apiKey.IPBlacklist) > 0) {
		...
		// Super keys are not bound to one group. Resolve the request's target
		// group from an explicit header/query parameter. Without a pin, the
		// scheduler receives an unbound Super Key and can select across groups.
		if apiKey.IsSuper() {
			c.Request = c.Request.WithContext(context.WithValue(c.Request.Context(), ctxkey.SuperAPIKey, true))
			var requestedGroupID *int64
			groupValue := strings.TrimSpace(c.GetHeader("X-Sub2API-Group-ID"))
			if groupValue == "" {
				groupValue = strings.TrimSpace(c.Query("group_id"))
			}
			if groupValue != "" {
				if parsed, parseErr := strconv.ParseInt(groupValue, 10, 64); parseErr != nil || parsed <= 0 {
					AbortWithError(c, http.StatusBadRequest, "INVALID_GROUP_ID", "Invalid target group id")
					return
				} else {
			...
			if requestedGroupID != nil {
				group, groupErr := apiKeyService.ResolveSuperAPIKeyGroup(c.Request.Context(), apiKey.User, requestedGroupID)
				if groupErr != nil {
					AbortWithError(c, http.StatusForbidden, "GROUP_NOT_ALLOWED", "No available target group for this API key")
					return
				}
				clone := *apiKey
				clone.Group = group
