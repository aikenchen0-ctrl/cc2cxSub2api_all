...
from api.licensed_finder_routes import router as licensed_finder_router
...
from automation.retry_policy import should_give_up
...
async def _run_automation_consumer_once() -> bool:
    ...
    try:
        ...
        max_attempts = int(getattr(settings, "automation_consumer_max_attempts", 3) or 3)
        if should_give_up(
            attempt_count=attempt_count,
            error_message=str(exc),
            max_attempts=max_attempts,
        ):
            queue.mark_failed(job_id, error_message=str(exc), finished_at=_now_iso())
            update_worker_state(
                "consumer",
                active_job_id="",
                last_activity_at=_now_iso(),
            ...
            logger.exception(
                "[AutomationConsumer] job=%s failed permanently after %s attempt(s): %s",
                job_id[:8],
                attempt_count,
                exc,
            )
