...
class TestLLMToolGenerate:
    ...
    @pytest.mark.asyncio
    async def test_generate_gpt55_forces_temperature_one(self):
        tool = LLMTool(settings=_make_settings(llm_model="gpt-5.5", llm_temperature=0.3))
        mock_resp = _mock_completion("ok")

        with patch("tools.llm_client.litellm.acompletion", new_callable=AsyncMock, return_value=mock_resp) as mock_call:
            await tool.generate("test", temperature=0)

        assert mock_call.call_args.kwargs["temperature"] == 1.0

    def test_apply_openai_api_mode_uses_responses_on_custom_gpt55(self):
        assert apply_openai_api_mode("gpt-5.5", mode="auto", api_base="http://api.cc2.cx/v1") == "openai/responses/gpt-5.5"
        assert apply_openai_api_mode("gpt-5.5", mode="chat", api_base="http://api.cc2.cx/v1") == "gpt-5.5"
        assert apply_openai_api_mode("gpt-4o-mini", mode="auto", api_base="http://api.cc2.cx/v1") == "gpt-4o-mini"
        assert apply_openai_api_mode("grok-4.6", mode="chat", api_base="http://api.cc2.cx/v1") == "openai/grok-4.6"
        assert apply_openai_api_mode("grok-4.5", mode="chat", api_base="http://api.cc2.cx/v1") == "openai/grok-4.5"
        assert apply_openai_api_mode("minimax/MiniMax-M2.5", mode="auto", api_base="http://api.cc2.cx/v1") == "minimax/MiniMax-M2.5"

    def test_llmtool_model_prefixes_responses_for_custom_gpt55(self):
        tool = LLMTool(settings=_make_settings(
            llm_model="gpt-5.5",
            openai_api_base="http://api.cc2.cx/v1",
            openai_api_mode="auto",
        ))
        assert tool.model == "openai/responses/gpt-5.5"

    def test_llmtool_prefixes_grok_on_custom_chat_gateway(self):
        tool = LLMTool(settings=_make_settings(
            llm_model="grok-4.6",
            openai_api_base="http://api.cc2.cx/v1",
            openai_api_mode="chat",
        ))
        assert tool.model == "openai/grok-4.6"

    def test_temperature_for_model_leaves_gpt4o(self):
        assert temperature_for_model("gpt-4o-mini", 0.3) == 0.3
        assert temperature_for_model("gpt-5.5", 0.3) == 1.0

    def test_reasoning_effort_for_gpt56_sol(self):
        assert reasoning_effort_for_model("openai/responses/gpt-5.6-sol", "low") == "low"
        assert reasoning_effort_for_model("gpt-4o-mini", "low") == ""
        assert reasoning_effort_for_model("gpt-5.6-sol", "nope") == ""

    @pytest.mark.asyncio
    async def test_generate_sends_low_effort_on_gpt56_sol(self):
        tool = LLMTool(settings=_make_settings(
        ...
        with patch("tools.llm_client.litellm.acompletion", new_callable=AsyncMock, return_value=mock_resp) as mock_call:
            await tool.generate("test")
        assert mock_call.call_args.kwargs["model"] == "openai/responses/gpt-5.6-sol"
        assert mock_call.call_args.kwargs["reasoning"] == {"effort": "low"}
...
class TestInjectApiKeys:
    def test_injects_keys_into_env(self):
        settings = _make_settings(
            ...
            openai_api_base="http://api.example.com/v1",
        ...
        env_keys = [
            "OPENAI_API_KEY", "OPENAI_API_BASE", "ANTHROPIC_API_KEY", "OPENROUTER_API_KEY",
        ...
        try:
            ...
            assert os.environ["OPENAI_API_BASE"] == "http://api.example.com/v1"
