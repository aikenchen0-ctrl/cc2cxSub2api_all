...
class TestGetSettings:
    ...
    async def test_returns_settings_shape(self, client):
        ...
        assert "search_readiness" in data
        assert "discovery_backend" in data["search_readiness"]
        assert "paid_maps" in data["search_readiness"]
        assert "warning" in data["search_readiness"]
        assert "new_hunt_note" in data["search_readiness"]
        assert "resume_note" in data["search_readiness"]
...
class TestSaveSettings:
    ...
    @pytest.mark.asyncio
    async def test_saves_email_guard_flags(self, client):
        with (
            patch("api.settings_routes.update_settings") as mock_update,
            patch("api.settings_routes.get_settings") as mock_gs,
        ):
            ...
            resp = await client.post(
                ...
                json={
                    "email_dry_run": "true",
                    "email_block_freemail": "false",
                    "email_min_minutes_between_sends": "12",
                    "email_verify_min_fit_score": "0.6",
                },
            )

        assert resp.status_code == 204
        updates = mock_update.call_args[0][0]
        assert updates["EMAIL_DRY_RUN"] == "true"
        assert updates["EMAIL_BLOCK_FREEMAIL"] == "false"
        assert updates["EMAIL_MIN_MINUTES_BETWEEN_SENDS"] == "12"
        assert updates["EMAIL_VERIFY_MIN_FIT_SCORE"] == "0.6"

    @pytest.mark.asyncio
    async def test_saves_osint_flags(self, client):
        with (
            patch("api.settings_routes.update_settings") as mock_update,
            patch("api.settings_routes.get_settings") as mock_gs,
        ):
            ...
            resp = await client.post(
                ...
                json={
                    ...
                    "builtwith_api_key": "bw-live-key",
                },
            )

        assert resp.status_code == 204
        updates = mock_update.call_args[0][0]
        assert updates["OSINT_ENRICHMENT_ENABLED"] == "true"
        assert updates["BUILTWITH_API_KEY"] == "bw-live-key"

    @pytest.mark.asyncio
    async def test_saves_social_presence_flags(self, client):
        with (
            patch("api.settings_routes.update_settings") as mock_update,
            patch("api.settings_routes.get_settings") as mock_gs,
        ):
            ...
            resp = await client.post(
                ...
                json={
                    ...
                    "social_presence_max_sites": "12",
                    "social_presence_country_tags": "de,cn",
                },
            )

        assert resp.status_code == 204
        updates = mock_update.call_args[0][0]
        assert updates["SOCIAL_PRESENCE_ENABLED"] == "true"
        assert updates["SOCIAL_PRESENCE_USE_MAIGRET"] == "false"
        assert updates["SOCIAL_PRESENCE_MAX_SITES"] == "12"
        assert updates["SOCIAL_PRESENCE_COUNTRY_TAGS"] == "de,cn"

    @pytest.mark.asyncio
    async def test_saves_licensed_finder_flags(self, client):
        with (
            patch("api.settings_routes.update_settings") as mock_update,
            patch("api.settings_routes.get_settings") as mock_gs,
        ):
            ...
            resp = await client.post(
                ...
                json={
                    ...
                    "bettercontact_api_key": "bc-live-key",
                    "licensed_finder_timeout_seconds": "180",
                },
            )

        assert resp.status_code == 204
        updates = mock_update.call_args[0][0]
        assert updates["LICENSED_FINDER_ENABLED"] == "true"
        assert updates["BETTERCONTACT_API_KEY"] == "bc-live-key"
        assert updates["LICENSED_FINDER_TIMEOUT_SECONDS"] == "180"

    @pytest.mark.asyncio
    async def test_saves_site_scan_flags(self, client):
        with (
            patch("api.settings_routes.update_settings") as mock_update,
            patch("api.settings_routes.get_settings") as mock_gs,
        ):
            ...
            resp = await client.post(
                ...
                json={
                    ...
                    "firecrawl_api_key": "fc-live-key",
                    "site_scan_max_pages": "8",
                },
            )

        assert resp.status_code == 204
        updates = mock_update.call_args[0][0]
        assert updates["SITE_SCAN_ENABLED"] == "true"
        assert updates["FIRECRAWL_API_KEY"] == "fc-live-key"
        assert updates["SITE_SCAN_MAX_PAGES"] == "8"
