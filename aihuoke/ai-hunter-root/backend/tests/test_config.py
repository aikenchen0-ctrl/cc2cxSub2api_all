...
from config.settings import Settings, get_settings, has_usable_api_key, sanitize_secret
...
class TestSettingsDefaults:
    ...
    def test_inline_comment_secrets_are_dropped(self):
        s = Settings(
            _env_file=None,
        ...
        assert s.openai_api_key == "sk-real"
        assert has_usable_api_key("# https://serper.dev") is False
        assert has_usable_api_key("xxx") is False
        assert sanitize_secret("  # note") == ""
        assert sanitize_secret("sk-real # comment") == "sk-real"
    ...
    def test_email_guard_defaults(self):
        s = Settings(_env_file=None)
        assert s.email_dry_run is True
        assert s.email_block_freemail is True
        assert s.email_min_minutes_between_sends == 0
        assert s.email_verify_min_fit_score == 0.0
        assert s.email_public_base_url == ""
        assert s.email_unsubscribe_secret == ""

    def test_osint_defaults_off(self):
        s = Settings(_env_file=None)
        assert s.osint_enrichment_enabled is False
        assert s.builtwith_api_key == ""

    def test_social_presence_defaults_off(self):
        s = Settings(_env_file=None)
        assert s.social_presence_enabled is False
        assert s.social_presence_use_maigret is True
        assert s.social_presence_max_sites == 20
        assert s.social_presence_country_tags == ""

    def test_licensed_finder_defaults_off(self):
        s = Settings(_env_file=None)
        assert s.licensed_finder_enabled is False
        assert s.bettercontact_api_key == ""
        assert s.licensed_finder_base_url == "https://app.bettercontact.rocks/api/v2"
        assert s.licensed_finder_timeout_seconds == 300

    def test_site_scan_defaults_off(self):
        s = Settings(_env_file=None)
        assert s.site_scan_enabled is False
        assert s.firecrawl_api_key == ""
        assert s.site_scan_max_pages == 6
    ...
    def test_file_upload_defaults(self):
        s = Settings(_env_file=None)
        assert Path(s.upload_dir).as_posix().endswith("/backend/uploads")
        assert Path(s.upload_dir).is_absolute()
        assert s.max_upload_size_mb == 50

    def test_checkpoint_db_default(self):
        s = Settings(_env_file=None)
        assert Path(s.checkpoint_db_path).as_posix().endswith("/backend/hunt_sessions.db")
