...
class TestGraphWithCustomNodes:
    ...
    def test_on_node_start_fires_before_failing_insight(self):
        started: list[str] = []

        def fake_insight(_state):
            raise RuntimeError("InsightAgent failed — aborting hunt")
        ...
        with pytest.raises(RuntimeError, match="InsightAgent failed"):
            graph.invoke(
                ...
                config={"configurable": {"thread_id": "test-fail-insight"}},
            )
        assert started == ["parse_description", "insight"]
