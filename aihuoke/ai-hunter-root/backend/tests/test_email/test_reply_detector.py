...
@pytest.mark.asyncio
async def test_reply_detector_treats_unsubscribe_reply_as_suppression(tmp_path: Path, monkeypatch):
    store = EmailStore(str(tmp_path / "email.db"))
    _seed_store(store)
    ...
    def fake_fetcher(account, *, now_iso: str):
        return [{
            "raw_ref": "<unsub-1@example.com>",
            "message_id": "<unsub-1@example.com>",
            "from_email": "buyer@acme.com",
            "subject": "Re: Hello",
            "in_reply_to": "<mid-1@example.com>",
            "references": ["<mid-1@example.com>"],
            "received_at": "2026-03-10T00:00:00Z",
            "snippet": "Please unsubscribe me from these emails.",
        }]
