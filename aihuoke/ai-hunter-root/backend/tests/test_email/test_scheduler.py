...
@pytest.mark.asyncio
async def test_scheduler_stops_suppressed_recipient(tmp_path: Path, monkeypatch):
    db_path = tmp_path / "email.db"
    store = EmailStore(str(db_path))
    store.init_db()
    store.upsert_account({
    ...
    async def fake_sender(*args, **kwargs):
        sent["n"] += 1
        return {"ok": True, "provider_message_id": "<mid>", "thread_key": "thread-1"}
    ...
    assert store.get_message("msg_1")["status"] == "failed"
...
@pytest.mark.asyncio
async def test_scheduler_fails_role_inbox(tmp_path: Path, monkeypatch):
    db_path = tmp_path / "email.db"
    store = EmailStore(str(db_path))
    store.init_db()
    store.upsert_account({
    ...
    async def fake_sender(*args, **kwargs):
        raise AssertionError("role inbox must not be sent")
    ...
    assert "role" in str(message.get("failure_reason") or "").lower()
