...
class TestCraftForLead:
    ...
    @pytest.mark.asyncio
    async def test_enrichment_fields_injected_into_prompt(self):
        sem = asyncio.Semaphore(3)
        llm = AsyncMock()
        llm.generate = AsyncMock(side_effect=[
            json.dumps({
            ...
            json.dumps({
                "recipient_profile": "Shopify merchant",
                "why_this_company_may_fit": ["Runs a Shopify storefront"],
                "best_value_angles": ["Fits industrial catalog"],
                "product_focus": ["switch"],
                "proof_points_to_use": ["Public GitHub org"],
                "claims_to_avoid": ["Avoid generic claims"],
                "cta_strategy": "Ask who owns supplier selection",
                "tone_guidance": "Professional",
                "personalization_hooks": ["Acme Tools"],
            }),
        ])
        captured = {}
        ...
        lead = {
            ...
            "reason": "Operator-only reason must stay out of outreach.",
        }
        insight = {"company_name": "MyCompany", "products": ["switch"]}
        ...
        assert "Acme Tools GmbH" in prompt
        assert "Shopify" in prompt
        assert "linkedin.com/company/acme" in prompt
        assert "Operator-only reason" not in prompt
