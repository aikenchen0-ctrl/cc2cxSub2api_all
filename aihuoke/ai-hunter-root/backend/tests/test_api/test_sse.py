...
class TestSseEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_stream_automation_job_keeps_heartbeat_while_idle(self, client, monkeypatch):
        fake_job = {
            "id": "job-idle",
            "status": "queued",
            "created_at": "2026-04-05T00:00:00+00:00",
            ...
            "template_seed_status": "pending",
            "template_seed_source": "",
            "payload": {"description": "Find distributors"},
        }
        ...
        class FakeQueue:
            ...
            def get(self, job_id):
                ...
                completed["status"] = "completed"
                completed["progress_stage"] = "completed"
                completed["progress_message"] = "done"
                return completed
        ...
        resp = await client.get("/api/v1/automation/jobs/job-idle/stream")
        assert resp.status_code == 200
        assert resp.text.count("event: heartbeat") >= 2
