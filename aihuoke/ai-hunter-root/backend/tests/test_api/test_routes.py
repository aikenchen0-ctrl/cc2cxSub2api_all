...
class TestExportCsv:
    @pytest.mark.asyncio
    async def test_export_csv_includes_reason_columns(self, client):
        _hunts["hunt-export"] = {
            "status": "completed",
            "result": {
                "leads": [
                    {
                        ...
                        "decision_makers": [
                            {"name": "Jane Doe", "title": "Purchasing Manager", "email": "jane.doe@acme.example"},
                        ],
                        "fit_reasons": ["Sells industrial hardware to distributors in SE Asia."],
                    }
                ]
            },
        }

        resp = await client.get("/api/v1/export.csv?hunt_id=hunt-export")
        assert resp.status_code == 200
        assert "text/csv" in resp.headers["content-type"]
        body = resp.text
        header = body.splitlines()[0]
        assert header == "email,first_name,last_name,company,title,website,linkedin_url,reason,lead_id,qualified_at"
        assert "jane.doe@acme.example" in body
        assert "Acme Tools" in body
        assert "Sells industrial hardware" in body

        named = await client.get("/api/v1/hunts/hunt-export/export.csv")
        assert named.status_code == 200
        assert "jane.doe@acme.example" in named.text

    @pytest.mark.asyncio
    async def test_export_csv_unknown_hunt(self, client):
        resp = await client.get("/api/v1/export.csv?hunt_id=missing")
        assert resp.status_code == 404


class TestExportReport:
    @pytest.mark.asyncio
    async def test_export_report_includes_keyword_effectiveness(self, client):
        _hunts["hunt-report"] = {
            "status": "completed",
            "result": {
                "insight": {"company_name": "SolarTech", "summary": "Inverters"},
                "used_keywords": ["solar inverter distributor DE"],
                "hunt_round": 1,
                "search_results": [{"url": "https://a.example"}] * 12,
                "keyword_search_stats": {
                    "solar inverter distributor DE": {"result_count": 12, "leads_found": 3},
                },
                "round_feedback": {
                    "round": 1,
                    ...
                    "industry_distribution": {"Energy": 3},
                    "region_distribution": {"DE": 3},
                },
                "leads": [
                    {
                        "company_name": "Acme",
                        "emails": ["secret@acme.example"],
                        "reason": "Operator-only reason",
                    }
                ],
            },
        }

        resp = await client.get("/api/v1/hunts/hunt-report/export-report.txt")
        assert resp.status_code == 200
        ...
        assert "25.0%" in body
        assert "高" in body
        assert "Operator-only reason" not in body
        assert "secret@acme.example" not in body

    @pytest.mark.asyncio
    async def test_export_report_unknown_hunt(self, client):
        resp = await client.get("/api/v1/hunts/missing/export-report.txt")
        assert resp.status_code == 404
...
class TestSlimState:
    ...
    def test_round_feedback_rebuilt_from_keyword_stats(self):
        ...
        assert fb["search_backend_error"] == ""

    def test_round_feedback_carries_search_backend_error(self):
        prior = _make_prior_result(keyword_search_stats={
            "solar inverter distributor": {
                "result_count": 0,
                "leads_found": 0,
                "error": "ddg_html: 502 Bad Gateway",
            },
        })
        state = _slim_state(prior, _make_resume_request())
        assert state["round_feedback"]["search_backend_error"] == "ddg_html: 502 Bad Gateway"
