...
from tools.web_search_router import WebSearcher, open_web_search
from tools.lead_contract import finalize_lead_contract, should_spend_verify_credits
from tools.osint_enrichment import enrich_leads
from tools.social_presence import enrich_leads_social
...
def _is_generic_mailbox(email: str) -> bool:
    ...
    normalized = str(email or "").replace("(inferred)", "").strip().lower()
    if not normalized or "@" not in normalized:
        return False
    if is_role_or_generic_email(normalized):
        return True
...
def _classify_email_pattern(name: str, email: str, domain: str) -> str | None:
    normalized = str(email or "").replace("(inferred)", "").strip().lower()
    return detect_email_pattern(normalized, name, domain)


def _render_email_pattern(name: str, pattern: str, domain: str) -> str | None:
    return apply_email_pattern(pattern, name, domain)
...
def _normalize_decision_maker_emails(lead: dict) -> dict:
    ...
    for item in decision_makers:
        ...
        if pattern:
            ...
            remember_domain_pattern(website_domain, pattern)

    chosen_pattern = exact_patterns[0] if exact_patterns else None
    if not chosen_pattern:
        ranked = ranked_keys_for_domain(website_domain)
        chosen_pattern = ranked[0] if ranked else None
...
async def _scrape_page(jina: JinaReaderTool, url: str) -> str:
    ...
    try:
        ...
        logger.debug("Failed to scrape %s via Jina: %s", url, e)
    fallback = await fetch_page_text(url)
    if fallback:
        logger.info("[LeadExtract] Jina empty — direct fetch returned %d chars for %s", len(fallback), url)
    return fallback
    ...
    *,
    ...
    async with semaphore:
        ...
        lead = {
            ...
            "fit_reasons": validated.get("fit_reasons", []) if isinstance(validated.get("fit_reasons", []), list) else [],
...
async def _verify_lead_emails(lead: dict, verifier: EmailVerifierTool) -> dict:
    ...
    if not emails:
        return lead
    if not should_spend_verify_credits(lead, get_settings()):
        return lead
    try:
...
async def lead_extract_node(state: HuntState) -> dict:
    ...
    google = open_web_search(allow_encyclopedia=False)
    ...
    try:
        new_leads = await enrich_leads(new_leads, settings=settings)
    except Exception as exc:
        logger.warning("[LeadExtractAgent] OSINT enrichment skipped: %s", exc)
    try:
        new_leads = await enrich_leads_social(new_leads, settings=settings)
    except Exception as exc:
        logger.warning("[LeadExtractAgent] social presence skipped: %s", exc)
