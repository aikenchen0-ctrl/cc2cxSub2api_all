...
from agents.keyword_strategy import MAPS_SYSTEM_PROMPT as KEYWORD_GEN_SYSTEM_PROMPT
...
from tools.search_limits import effective_keywords_per_round
...
def _build_prompt(state: HuntState) -> str:
    ...
    if feedback and isinstance(feedback, dict):
        ...
        if search_backend_error:
            parts.append(
                "## Search backend blocked\n"
                f"{search_backend_error}\n"
                "This is a network/backend failure, not a keyword miss. "
                "Do not treat zero results as weak keyword patterns."
            )
...
async def keyword_gen_node(state: HuntState) -> dict:
    ...
    n_keywords = effective_keywords_per_round(
        settings,
        state.get("keywords_per_round"),
    )
