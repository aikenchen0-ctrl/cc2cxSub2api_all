...
from tools.site_scanner import scan_site
...
def _build_user_prompt(
    ...
    # ── 2b. Pre-crawled high-value site brief (optional, default off) ─
    brief_md = ""
    if isinstance(site_brief, dict) and not site_brief.get("skipped"):
        brief_md = str(site_brief.get("brief_markdown") or "").strip()
    if brief_md:
        provider = site_brief.get("provider") or "crawler"
        page_n = len(site_brief.get("pages") or [])
        parts.append(
            f"## Pre-crawled Site Brief ({page_n} high-value page(s) via {provider})\n"
            f"These pages were already ranked and fetched (pricing / about / product). "
            f"Treat them as primary evidence. You may still scrape_page for a missing subpage, "
            f"but do not re-scrape pages already included.\n\n{brief_md}"
        )
...
async def insight_node(state: HuntState) -> dict:
    ...
    # ── Step 1b: Optional high-value site scan (default off, never aborts) ──
    site_brief: dict | None = None
    if website_url:
        try:
            site_brief = await scan_site(website_url, settings=get_settings())
            if site_brief.get("skipped"):
                logger.info(
                    "[InsightAgent] Site scan skipped (%s)",
                    site_brief.get("reason") or "disabled",
                )
            else:
                logger.info(
                    "[InsightAgent] Site scan %s via %s — %d page(s)",
                    site_brief.get("domain") or website_url,
                    site_brief.get("provider") or "?",
                    len(site_brief.get("pages") or []),
                )
        except Exception as scan_err:
            logger.warning("[InsightAgent] Site scan failed (non-fatal): %s", scan_err)
            site_brief = None
