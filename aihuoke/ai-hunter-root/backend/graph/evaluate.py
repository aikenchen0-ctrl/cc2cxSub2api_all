...
2. should_continue_hunting(): decide "continue" or "finish" based on stop conditions
"""
...
from tools.search_backend_error import round_search_is_blocked
...
def evaluate_progress(state: HuntState) -> dict:
    ...
    search_backend_error = round_search_is_blocked(keyword_stats, state.get("keywords"))
    ...
    round_feedback = {
        ...
        "search_backend_error": search_backend_error,
    }
...
def should_continue_hunting(state: HuntState) -> str:
    ...
    3. Search backend blocked and this round added 0 leads
    4. Diminishing returns: new leads this round < configured min_threshold (default 5)
    ...
    if isinstance(feedback, dict):
        ...
        search_backend_error = str(feedback.get("search_backend_error") or "").strip()
    else:
    ...
    # Stop condition 3: public web search is blocked. Extra keyword rounds
    # cannot recover; this also covers resume hunts that already have old leads
    # (the zero-leads rule would not fire).
    if search_backend_error and new_this_round == 0:
        logger.info(
            "[Evaluate] FINISH — search backend blocked after round %d (%s)",
            evaluated_round,
            search_backend_error,
        )
        return "finish"

    # Stop condition 4: diminishing returns (skip on round 1)
    ...
    if evaluated_round >= 1 and current_leads == 0 and new_this_round == 0:
        logger.info(
            "[Evaluate] FINISH — zero leads after round %d (search returned nothing usable)",
            evaluated_round,
        )
        return "finish"
