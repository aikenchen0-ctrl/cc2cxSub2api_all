...
CREATE INDEX IF NOT EXISTS idx_suppressions_created ON email_suppressions(created_at);
"""
...
class EmailStore:
    ...
    def init_db(self) -> None:
        with self._connect() as conn:
            ...
            self._ensure_unsubscribe_secret(conn)
    ...
    def _ensure_unsubscribe_secret(self, conn: sqlite3.Connection) -> str:
        row = conn.execute(
            "SELECT value FROM email_meta WHERE key = ?",
            ("unsubscribe_secret",),
        ).fetchone()
        ...
        conn.execute(
            "INSERT OR IGNORE INTO email_meta(key, value) VALUES (?, ?)",
        ...
        return str(row["value"]) if row else secret
    ...
    def unsubscribe_secret(self) -> str:
        with self._connect() as conn:
            return self._ensure_unsubscribe_secret(conn)
    ...
    def is_suppressed(self, email: str) -> bool:
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT email FROM email_suppressions WHERE email = ?",
                (normalised,),
            ).fetchone()
        return row is not None

    def get_suppression(self, email: str) -> dict[str, Any] | None:
        normalised = str(email or "").strip().lower()
        if not normalised:
            return None
        with self._connect() as conn:
    ...
    def add_suppression(
        ...
        *,
        ...
        reason_text = str(reason or "unsubscribed").strip() or "unsubscribed"
        source_text = str(source or "recipient").strip() or "recipient"
        with self._connect() as conn:
            conn.execute(
                """
                ...
                """,
                (normalised, reason_text, source_text, created_at, created_at),
            )
        return self.get_suppression(normalised) or {
            "email": normalised,
            "reason": reason_text,
            "source": source_text,
    ...
    def list_sequences_for_email(self, email: str) -> list[dict[str, Any]]:
        normalised = str(email or "").strip().lower()
        if not normalised:
            return []
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM lead_email_sequences WHERE lower(lead_email) = ? ORDER BY created_at ASC",
                (normalised,),
            ).fetchall()
        return [dict(row) for row in rows]
