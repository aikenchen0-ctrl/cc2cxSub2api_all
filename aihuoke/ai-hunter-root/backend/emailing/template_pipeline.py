...
from tools.lead_enrichment_view import enrichment_hooks, format_enrichment_block
...
def build_fallback_template_profile(
    *,
    ...
    extra_hooks = enrichment_hooks(lead)
    return {
        ...
        "template_notes": (
            ...
            + (f" Enrichment: {'; '.join(extra_hooks)}." if extra_hooks else "")
        ),
    }
...
async def extract_template_profile(
    ...
    *,
    ...
    prompt = (
        ...
        f"{format_enrichment_block(lead)}\n"
        f"</buyer>\n\n"
        f"<notes>\n{notes}\n</notes>\n\n"
