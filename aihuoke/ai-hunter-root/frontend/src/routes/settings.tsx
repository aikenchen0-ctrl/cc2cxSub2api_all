...
const PROVIDERS: Provider[] = [
  {
    ...
    defaultModels: ["grok-4.5", "grok-4.6", "gpt-5.6-sol", "gpt-5.5", "gpt-4o-mini", "gpt-4o"],
    reasoningModels: ["grok-4.5", "grok-4.6", "gpt-5.6-sol", "gpt-5.5", "gpt-4o", "o1-mini"],
  },
  {
    id: "anthropic",
    label: "Anthropic",
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        {provider.id === "openai" && (
          <div className="space-y-1.5">
            <Label htmlFor="openai-api-base">OpenAI 兼容 Base URL</Label>
            <Input
              id="openai-api-base"
              value={values.openai_api_base ?? ""}
              onChange={(e) => onChange("openai_api_base", e.target.value)}
              placeholder="https://api.openai.com/v1"
            />
            <p className="text-xs text-muted-foreground">
              自定义网关填到 /v1，例如 http://api.example.com/v1。留空则走官方 OpenAI。
            </p>
          </div>
        )}
        ...
        {provider.id === "openai" && (
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  key={value}
                  type="button"
                  onClick={() => onChange("openai_api_mode", value)}
                  className={`rounded-md border px-3 py-2 text-sm ${
                    (values.openai_api_mode || "auto") === value
                      ? "border-primary bg-primary/10 text-primary"
            ...
            <p className="text-xs text-muted-foreground">
              调用方仍发聊天消息。gpt-5.6-sol / gpt-5.5 这类推理模型在自定义网关上默认走 /v1/responses；需要旧聊天接口时再选 Chat Completions。
            </p>
          </div>
        )}
        ...
        {provider.id === "openai" && (
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {["", "none", "low", "medium", "high"].map((value) => (
                <button
                  key={value || "default"}
                  type="button"
                  onClick={() => onChange("llm_reasoning_effort", value)}
                  className={`rounded-md border px-3 py-2 text-sm ${
                    (values.llm_reasoning_effort || "") === value
                      ? "border-primary bg-primary/10 text-primary"
                      : "border-border text-muted-foreground"
                  }`}
                >
            ...
            <p className="text-xs text-muted-foreground">
              只作用于 GPT-5.x。low 更快更便宜，适合先跑通 Insight。
            </p>
          </div>
        )}
...
const SEARCH_FIELDS: FieldDef[] = [
  { key: "search_backend", label: "搜索后端", placeholder: "auto", hint: "auto=有 Serper 走 Maps，无 Key 走 DuckDuckGo；serper=强制 Maps；free=强制免费网页搜索" },
  { key: "tavily_api_key", label: "Tavily API Key", placeholder: "tvly-…（多 Key 用逗号分隔）", secret: true, hint: "通用网页搜索，支持多 Key：key1,key2" },
  { key: "serper_api_key", label: "Serper API Key", placeholder: "", secret: true, hint: "Google Maps 搜索。不填则走 DuckDuckGo；本机被墙时会明确失败，不会把百科当线索" },
  { key: "search_http_proxy", label: "搜索 HTTP 代理", placeholder: "http://127.0.0.1:7897", secret: false, hint: "免费网页搜索走这个代理，跳过 WorkBuddy 沙箱 60376。Clash Verge 一般是 7897" },
  { key: "jina_api_key", label: "Jina Reader API Key", placeholder: "", secret: true, hint: "网页读取与抓取。可不填，公开 r.jina.ai 限流裸跑" },
...
const EMAIL_DELIVERY_FIELDS: FieldDef[] = [
  ...
  { key: "email_public_base_url", label: "对外访问地址", placeholder: "https://hunter.example.com", hint: "退订链接用的公网地址；留空则用 http://127.0.0.1:8000" },
];
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base">可选 enrichment 模块</CardTitle>
        </div>
        <CardDescription>
          全部默认关闭，不进主搜索图。打开后才会在 LeadExtract / Insight 收尾补档案，失败不影响 hunt。
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="rounded-md border px-4 py-4 space-y-4">
          <div className="space-y-2">
            <Label>OSINT 补全（公开网页 / email-format / BuiltWith）</Label>
            <BoolToggle
              value={values.osint_enrichment_enabled}
              onChange={(value) => onChange("osint_enrichment_enabled", value)}
            />
            <p className="text-xs text-muted-foreground">
              已有个人邮箱的线索会跳过 email-format 再搜，避免重复烧积分。
            </p>
          </div>
          <div className="space-y-1.5">
            ...
            <SecretInput
              ...
              value={values.builtwith_api_key ?? ""}
              onChange={(value) => onChange("builtwith_api_key", value)}
              placeholder="可选；不填则只做页面技术栈探测"
            />
          </div>
        </div>
        ...
        <div className="rounded-md border px-4 py-4 space-y-4">
          <div className="space-y-2">
            <Label>社媒存在性（maigret 白名单，每条 ≤20 站）</Label>
            <BoolToggle
              value={values.social_presence_enabled}
              onChange={(value) => onChange("social_presence_enabled", value)}
            />
          </div>
          <div className="space-y-2">
            <Label>优先走 maigret 库</Label>
            <BoolToggle
              value={values.social_presence_use_maigret}
              onChange={(value) => onChange("social_presence_use_maigret", value)}
              defaultValue="true"
            />
            <p className="text-xs text-muted-foreground">未安装 maigret 时自动降级为有界 HTTP probe。</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label htmlFor="social_presence_max_sites">每条线索站点数上限</Label>
              <Input
                id="social_presence_max_sites"
                value={values.social_presence_max_sites ?? ""}
                onChange={(e) => onChange("social_presence_max_sites", e.target.value)}
                placeholder="20"
                className="font-mono text-sm"
              />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="social_presence_country_tags">国家 tags</Label>
              <Input
                id="social_presence_country_tags"
                value={values.social_presence_country_tags ?? ""}
                onChange={(e) => onChange("social_presence_country_tags", e.target.value)}
                placeholder="cn,de"
                className="font-mono text-sm"
              />
            </div>
        ...
        <div className="rounded-md border px-4 py-4 space-y-4">
          <div className="space-y-2">
            <Label>持牌找人旁路（BetterContact，不进主图）</Label>
            <BoolToggle
              value={values.licensed_finder_enabled}
              onChange={(value) => onChange("licensed_finder_enabled", value)}
            />
            <p className="text-xs text-muted-foreground">
              只走独立 API，不进主 hunt 图。搜索默认不买邮箱，enrich 必须显式打开才花 credits。
              {" "}
              <Link to="/licensed-finder" className="text-primary underline underline-offset-2">
                打开持牌找人页
              </Link>
            </p>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="bettercontact_api_key">BetterContact API Key</Label>
            <SecretInput
              id="bettercontact_api_key"
              value={values.bettercontact_api_key ?? ""}
              onChange={(value) => onChange("bettercontact_api_key", value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="licensed_finder_timeout_seconds">超时（秒）</Label>
            <Input
              id="licensed_finder_timeout_seconds"
              value={values.licensed_finder_timeout_seconds ?? ""}
              onChange={(e) => onChange("licensed_finder_timeout_seconds", e.target.value)}
              placeholder="300"
              className="font-mono text-sm"
            />
          </div>
        ...
        <div className="rounded-md border px-4 py-4 space-y-4">
          <div className="space-y-2">
            <Label>Insight 站点简报（Firecrawl 高价值页）</Label>
            <BoolToggle
              value={values.site_scan_enabled}
              onChange={(value) => onChange("site_scan_enabled", value)}
            />
            <p className="text-xs text-muted-foreground">
              无 key 时走普通 fetch。每站只扫高价值路径，不爬整站。
            </p>
          </div>
          <div className="space-y-1.5">
            ...
            <SecretInput
              ...
              onChange={(value) => onChange("firecrawl_api_key", value)}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="site_scan_max_pages">每站页数上限</Label>
            <Input
              id="site_scan_max_pages"
              value={values.site_scan_max_pages ?? ""}
              onChange={(e) => onChange("site_scan_max_pages", e.target.value)}
              placeholder="6"
              className="font-mono text-sm"
            />
          </div>
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          <Shield className="h-5 w-5 text-primary" />
          <CardTitle className="text-base">发信护栏</CardTitle>
        </div>
        <CardDescription>
          dry-run、免费邮箱拦截、发送窗口和日限额。默认偏保守，出站前务必确认。
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="space-y-2">
          <Label>Dry-run（只模拟、不出站）</Label>
          <BoolToggle
            value={values.email_dry_run}
            onChange={(value) => onChange("email_dry_run", value)}
            trueLabel="开启"
            falseLabel="关闭（真发）"
            defaultValue="true"
          />
        ...
        <div className="space-y-2">
          <Label>拦截免费邮箱域名</Label>
          <BoolToggle
            value={values.email_block_freemail}
            onChange={(value) => onChange("email_block_freemail", value)}
            defaultValue="true"
          />
        </div>
        <div className="space-y-2">
          <Label>仅工作日发送</Label>
          <BoolToggle
            value={values.email_weekdays_only}
            onChange={(value) => onChange("email_weekdays_only", value)}
            defaultValue="true"
          />
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="email_business_hours_start">发送窗口开始</Label>
            <Input
              id="email_business_hours_start"
              value={values.email_business_hours_start ?? ""}
              onChange={(e) => onChange("email_business_hours_start", e.target.value)}
              placeholder="09:00"
              className="font-mono text-sm"
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="email_business_hours_end">发送窗口结束</Label>
            <Input
              id="email_business_hours_end"
              value={values.email_business_hours_end ?? ""}
              onChange={(e) => onChange("email_business_hours_end", e.target.value)}
              placeholder="18:00"
              className="font-mono text-sm"
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="email_daily_send_limit">日发送上限</Label>
            <Input
              id="email_daily_send_limit"
              value={values.email_daily_send_limit ?? ""}
              onChange={(e) => onChange("email_daily_send_limit", e.target.value)}
              placeholder="50"
              className="font-mono text-sm"
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="email_hourly_send_limit">小时发送上限</Label>
            <Input
              id="email_hourly_send_limit"
              value={values.email_hourly_send_limit ?? ""}
              onChange={(e) => onChange("email_hourly_send_limit", e.target.value)}
              placeholder="10"
              className="font-mono text-sm"
            />
          </div>
          <div className="space-y-1.5 md:col-span-2">
            <Label htmlFor="email_min_minutes_between_sends">两次发送最小间隔（分钟）</Label>
            <Input
              id="email_min_minutes_between_sends"
              value={values.email_min_minutes_between_sends ?? ""}
              onChange={(e) => onChange("email_min_minutes_between_sends", e.target.value)}
              placeholder="0"
              className="font-mono text-sm"
            />
          </div>
...
function SuppressionPanel() {
  ...
  const [reason, setReason] = useState("operator");

  const listQuery = useQuery({
    queryKey: ["email-suppressions"],
    queryFn: () => api.listSuppressions(200),
  });

  const addMutation = useMutation({
    mutationFn: () => api.addSuppression(email.trim(), reason.trim() || "operator"),
    onSuccess: () => {
      setEmail("");
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base">退订 / 抑制名单</CardTitle>
        </div>
        <CardDescription>
          命中名单的地址不会再发。公开退订走 HMAC 链接；这里给运营手动加入。
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-[1fr_160px_auto]">
          ...
          >
            {addMutation.isPending ? "加入中…" : "加入名单"}
          </Button>
        </div>
        {addMutation.isSuccess && (
          <p className="text-sm text-emerald-600">
            已抑制 {addMutation.data.email}，停序列 {addMutation.data.sequences_stopped} 条
          </p>
        )}
        {addMutation.isError && (
        ...
        {listQuery.isLoading ? (
          <p className="text-sm text-muted-foreground">正在加载名单…</p>
        ) : items.length === 0 ? (
          <p className="text-sm text-muted-foreground">名单为空。</p>
        ) : (
  ...
  useEffect(() => {
    if (data?.settings) {
      ...
      const keyMap: Record<string, string> = {
        ...
        SEARCH_HTTP_PROXY: "search_http_proxy",
        ...
        OSINT_ENRICHMENT_ENABLED: "osint_enrichment_enabled",
        BUILTWITH_API_KEY: "builtwith_api_key",
        SOCIAL_PRESENCE_ENABLED: "social_presence_enabled",
        SOCIAL_PRESENCE_USE_MAIGRET: "social_presence_use_maigret",
        SOCIAL_PRESENCE_MAX_SITES: "social_presence_max_sites",
        SOCIAL_PRESENCE_COUNTRY_TAGS: "social_presence_country_tags",
        LICENSED_FINDER_ENABLED: "licensed_finder_enabled",
        BETTERCONTACT_API_KEY: "bettercontact_api_key",
        LICENSED_FINDER_TIMEOUT_SECONDS: "licensed_finder_timeout_seconds",
        SITE_SCAN_ENABLED: "site_scan_enabled",
        FIRECRAWL_API_KEY: "firecrawl_api_key",
        SITE_SCAN_MAX_PAGES: "site_scan_max_pages",
        EMAIL_PUBLIC_BASE_URL: "email_public_base_url",
        EMAIL_DRY_RUN: "email_dry_run",
        EMAIL_BLOCK_FREEMAIL: "email_block_freemail",
        EMAIL_MIN_MINUTES_BETWEEN_SENDS: "email_min_minutes_between_sends",
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">配置模型、搜索、邮件发送、回信检测和可选 enrichment 模块。</p>
      </div>
      ...
      {data?.search_readiness?.warning && (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-100">
          {[data.search_readiness.warning, data.search_readiness.new_hunt_note]
            .map((item) => (item || "").trim())
            .filter(Boolean)
            .join(" ")}
        </div>
      ...
      <FieldGroup
        ...
        description="不填 Serper 走 DuckDuckGo（无 Key，质量降档；被墙会明确失败，百科不当线索）。填了 Serper 才走 Google Maps。Tavily 仍用于通用网页检索。"
      ...
      <EmailGuardsPanel values={values} onChange={handleChange} />

      <SuppressionPanel />

      <OptionalModulesPanel values={values} onChange={handleChange} />
