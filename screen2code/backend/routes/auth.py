...
from auth import consume_ticket, create_session, decrypt_relay_key, get_identity, logout, require_csrf, revoke_sessions_for_subject, _safe_next, verify_ticket
...
async def service_status(request: Request):
    ...
    if not SCREEN2CODE_AUTH_REQUIRED:
        return {"status": "ready"}
    if decrypt_relay_key(identity.relay_key_ciphertext) and SUB2API_RELAY_BASE_URL:
        return {"status": "ready"}
