...
import { exchangeSub2APISession, fetchCurrentUser } from "@/services/api/auth";
...
function LoginContent() {
    ...
    useEffect(() => {
        ...
        void exchangeSub2APISession().then(({ token, user }) => {
            setSession(token, user);
            useConfigStore.getState().updateConfig("channelMode", "remote");
            router.replace(redirect);
            router.refresh();
        }).catch((error: unknown) => {
            message.error(error instanceof Error ? error.message : "Sub2API 登录失败");
        ...
        <main className="flex h-full min-h-0 items-center justify-center overflow-y-auto bg-background bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-6 py-10 [background-size:16px_16px] dark:bg-[radial-gradient(rgba(245,245,244,.16)_1px,transparent_1px)]">
            <section className="w-full max-w-[420px]">
                <div className="mb-7 text-center">
                    <img src="/project-icon.jpg" data-sub2api-site-logo alt="AI生图生视频" className="mx-auto mb-4 block size-12 rounded-xl object-cover" />
