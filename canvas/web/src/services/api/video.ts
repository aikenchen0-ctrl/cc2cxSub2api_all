...
import { isLegacyGatewayVideoModel, legacyGatewayVideoBody } from "@/lib/legacy-gateway-video";
...
async function cacheProtectedVideo(config: AiConfig, model: string, task: VideoResponse) {
    ...
    const needsSub2APIContent = task.channelId === "sub2api-relay" && (!url || /\/videos\/[^/]+\/content(?:[?#]|$)/.test(url));
    const needsGrokContent = isGrok2APIVideoConfig(config, model) && /\/v1\/videos\/[^/]+\/content(?:[?#]|$)/.test(url);
    if (!isCompletedVideoStatus(task.status) || task.storageKey || (!needs88APIContent && !needsGrokContent && !needsSub2APIContent)) return task;
    const taskId = needsSub2APIContent ? task.id : task.task_id || task.id || task.video_id || "";
...
async function createVideoRequestBody(config: AiConfig, model: string, prompt: string, input: Required<VideoReferenceInput>) {
    if (isLegacyGatewayVideoModel(model, videoChannelProtocol(config, model))) {
        ...
        return legacyGatewayVideoBody({
            model, prompt, seconds: config.videoSeconds, size: config.size, resolution: config.vquality,
            images: await Promise.all(input.references.slice(0, 8).map(imageToDataUrl)),
