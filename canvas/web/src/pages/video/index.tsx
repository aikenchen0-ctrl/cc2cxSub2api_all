...
export default function VideoPage() {
    ...
    const retryResult = () => {
        const retryLog = previewLog?.task
            ? previewLog
            : logs.find((item) => item.task && (item.status === "failed" || item.status === "pending"));
        if (retryLog?.task) {
            const resumedLog = { ...retryLog, status: "pending" as const, error: undefined };
            setResults([{ id: resumedLog.id, status: "pending" }]);
            void saveLog(resumedLog, false).then(() => pollGenerationLog(resumedLog));
            return;
        }
        void generate();
    };
