...
func New() *gin.Engine {
	router := gin.New()
	// Query strings can contain one-time SSO tickets or signed media URLs.
	router.Use(gin.LoggerWithConfig(gin.LoggerConfig{Formatter: func(p gin.LogFormatterParams) string {
		path, _, _ := strings.Cut(p.Path, "?")
		return fmt.Sprintf("%s %s %d\n", p.Method, path, p.StatusCode)
	}}), gin.Recovery())
