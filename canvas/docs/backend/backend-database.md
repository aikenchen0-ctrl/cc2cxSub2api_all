...
- users.sub2api_subject：外部身份标识（索引），不在用户 JSON 中返回。SSO 用户 ID 由 subject 的 SHA-256 确定，不按同名邮箱/用户名合并已有账号。
- sso_tickets：digest 为已使用 jti 的 SHA-256（主键），expires_at 为过期 Unix 秒（索引）。插入唯一摘要原子地消费票据；后续消费时清理过期记录。所有副本需共享数据库。
- settings.private.channels 中 ID 为 sub2api-relay 的渠道由启动环境配置同步；密钥不出现在公开响应中。
