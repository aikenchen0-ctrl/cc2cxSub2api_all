...
func TestModelProtocolChannelFlow(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Sub2API text", protocol: "openai", modelName: "gpt-5.5", endpoint: "/chat/completions", body: `{"model":"gpt-5.5","messages":[]}`, path: "/v1/chat/completions", payload: `{"choices":[]}`},
		{name: "Sub2API image", protocol: "openai", modelName: "grok-imagine-image", endpoint: "/images/generations", body: `{"model":"grok-imagine-image","prompt":"scene"}`, path: "/v1/images/generations", payload: `{"data":[{"b64_json":"AQID"}]}`},
		{name: "Sub2API video", protocol: "openai", modelName: "minimax_h3_b99_001", endpoint: "/videos", body: `{"model":"minimax_h3_b99_001","prompt":"scene","seconds":6}`, path: "/v1/videos", payload: `{"id":"upstream-job","status":"processing"}`},
		{name: "Amam video create and poll", protocol: "openai", modelName: "xinghe-2.0", endpoint: "/videos", body: `{"model":"xinghe-2.0","prompt":"scene","seconds":"5","size":"16:9","resolution":"480p","video_reference[]":["https://media.invalid/ref.mp4"]}`, path: "/v1/videos", wantBody: `{"model":"xinghe-2.0","prompt":"scene","seconds":"5","size":"16:9","resolution":"480p","video_reference[]":["https://media.invalid/ref.mp4"]}`, payload: `{"id":"upstream-job","status":"processing"}`, pollPath: "/v1/videos/upstream-job", pollPayload: `{"id":"upstream-job","status":"completed","video_url":"https://media.invalid/video"}`},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if strings.HasPrefix(test.name, "Sub2API") {
				channel.ID = "sub2api-relay"
			}
