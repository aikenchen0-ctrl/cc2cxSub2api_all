...
function decryptRelayKey(ciphertext: string): string {
...
export function verifyAicutSSOTicket(raw: string, now = Math.floor(Date.now() / 1000)): VerifiedAicutSSO {
  ...
  if (ticket.iss !== 'sub2api' || ticket.aud !== 'openchatcut'
    ...
    || !/^[a-f0-9-]{16,128}$/i.test(ticket.jti) || !ticket.rk || ticket.rk.length > 4096) {
  ...
  const relayKey = decryptRelayKey(ticket.rk);
  if (!relayKey) throw new Error('SSO ticket has no relay key');
