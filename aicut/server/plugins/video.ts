...
function satelliteVideoResultUrl(current: Record<string, unknown>): string {
  const video = current.video as { url?: unknown } | undefined;
  const content = current.content as { video_url?: unknown } | undefined;
  const raw = current.url ?? current.download_url ?? current.video_url ?? video?.url ?? content?.video_url;
...
async function generateSatelliteVideo(
  input: ValidVideoRequest,
  registerProviderTask: RegisterGenerationProviderTask,
  existingTaskId?: string,
  ownerUserId?: string,
): Promise<string> {
  const headers = satelliteHeadersForUser(ownerUserId);
  const base = satelliteV1Base();
  if (!headers || !base) throw new Error('Sub2API satellite is not ready');
  const authHeaders = { ...headers, 'Content-Type': 'application/json' };
  let taskId = existingTaskId;
  if (!taskId) {
    const started = await requestJson(`${base}/videos`, {
      method: 'POST',
      headers: authHeaders,
      body: JSON.stringify({
        model: satelliteVideoModel(input.model),
        prompt: input.prompt,
        seconds: input.durationSeconds,
        size: input.ratio,
      }),
    });
    taskId = String(started.id ?? started.request_id ?? '');
    if (!taskId) throw new Error('Sub2API video did not return a task id');
    await registerProviderTask(input.model, taskId);
  }
  const deadline = Date.now() + 15 * 60_000;
  while (Date.now() < deadline) {
    const current = await requestJson(`${base}/videos/${encodeURIComponent(taskId)}`, { headers: authHeaders });
    const status = String(current.status ?? '').toLowerCase();
    if (status === 'done' || status === 'completed' || status === 'succeeded') {
      const url = satelliteVideoResultUrl(current);
      if (!url) throw new Error('Sub2API video succeeded without a URL');
      return url;
    }
    if (VIDEO_FAILURES.has(status) || status === 'failed' || status === 'expired' || status === 'cancelled') {
      throw new Error(`Sub2API video generation ${status}`);
    }
    await wait(3_000);
  }
  throw new Error('Sub2API video generation timed out');
}
...
async function runVideoOperation(
  ...
  const satelliteUser = ownerUserId || currentTenant()?.userId;
  if (!checkpoint.complete) {
    if (satelliteHeadersForUser(satelliteUser) && satelliteV1Base()) {
      const url = await generateSatelliteVideo(input, registerProviderTask, providerTaskId, satelliteUser);
