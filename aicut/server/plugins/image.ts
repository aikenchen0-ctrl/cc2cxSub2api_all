...
async function callOpenaiViaSdk(baseUrl: string, apiKey: string, body: GptImageInput, extraHeaders?: Record<string, string>): Promise<ProviderImage[]> {
  ...
  const openai = createOpenAI({
    ...
    headers: extraHeaders
      ? Object.fromEntries(Object.entries(extraHeaders).filter(([key]) => key.toLowerCase() !== 'authorization'))
      : undefined,
  });
...
async function callProvider(baseUrl: string, apiKey: string, body: GptImageInput, extraHeaders?: Record<string, string>): Promise<ProviderImage[]> {
  if (!body.referencePaths.length) return callOpenaiViaSdk(baseUrl, apiKey, body, extraHeaders);
  const endpoint = '/v1/images/edits';
  let requestBody: string | FormData;
  let headers: Record<string, string> = { Authorization: `Bearer ${apiKey}`, ...extraHeaders };
    ...
    configureServer(server) {
      server.middlewares.use('/generate/image', async (req, res) => {
        ...
        try {
          ...
          const satellite = satelliteHeaders();
          if (satellite && satelliteReady()) {
            images = await callProvider(satelliteOrigin(), satelliteCredential(), {
              model: satelliteImageModel(model), prompt, quality, count, size: `${width}x${height}`, referencePaths, maskPath,
              background, moderation, inputFidelity, outputFormat, outputCompression,
            }, satellite);
