...
<div data-tips="true" data-tips-type="danger">Seedance 2.5 已全面公开，您可在火山方舟平台调用 API 及在线体验。调用模型前，<strong>请务必仔细查阅 </strong><a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_compatibility"><strong>使用必读</strong></a><strong>，以确保正确设置任务类型和配置参数</strong>。</div>

<div data-tips="true" data-tips-type="warning" data-tips-is-title="true">限时优惠</div>

<div data-tips="true" data-tips-type="warning">Seedance 2.0 mini 与 Seedance 2.0 fast 现已开启限时优惠活动，优惠<strong>仅适用于企业用户</strong>，<strong>且达到规定的 token 用量上限后将恢复按刊例价结算</strong>，规则详见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-0-mini-fast-limited-time-discount">Seedance 模型限时折扣活动规则</a>。</div>
...
详细规则见 [Seedance 2.5 与 Seedance 2.0 系列模型开通、使用与退订说明](https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-model-activation-usage-and-refund)。
...
* <div data-tips="true" data-tips-type="tip">不同模型对应支持不同的参数与取值，详见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/video-generation-tutorial#9fe4cce0">输出视频格式</a>。当输入的参数或取值不符合所选的模型时，该参数将被忽略或触发报错：</div>
...
您需要调用的模型的 ID（Model ID）。[开通模型服务](https://ark.volcengine.com/region:cn-beijing/openManagement?LLM=%7B%7D&OpenTokenDrawer=false) 后可 [查询 Model ID](https://ark.volcengine.com/region:cn-beijing/docs/ark/model-list)。
...
<div data-tips="true" data-tips-type="warning">为了便利创作者对肖像的使用，平台推出了以下解决方案，详情参见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-0#5c67c9a1">教程</a>。</div>
...
* <div data-tips="true" data-tips-type="tip"><strong>更多使用技巧</strong>：提示词的详细使用技巧，请参见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-0-prompt-guide">Seedance 提示词指南</a>。</div>
...
* <div data-tips="true" data-tips-type="tip"><strong>格式</strong>：<code>jpeg</code>、<code>png</code>、<code>webp</code>、<code>bmp</code>、<code>tiff</code>、<code>gif</code>。其中，Seedance 2.0 系列及以上模型版本额外支持 <code>heic</code>、<code>heif</code>。</div>
...
详情参见 [教程](https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-portrait-asset-guide#trust-model-output)。
...
* <div data-tips="true" data-tips-type="tip"><strong>时长</strong>：</div>
  ...
  * <div data-tips="true" data-tips-type="tip"><strong>Seedance 2.5</strong>：</div>
    ...
    * <div data-tips="true" data-tips-type="tip"><a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_task_type_intro">视频编辑任务</a>：单个视频时长 <code>[4, 30]</code> s。</div>
...
* 默认情况下，即 `omni_reference_task_type=auto`：模型根据输入素材和提示词自动判定任务类型，再校验参数取值。如果参数与实际任务类型不兼容，任务将触发 [异步报错](https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_async_error)（错误码：[InvalidParameter.TaskTypeConstraint](https://ark.volcengine.com/region:cn-beijing/docs/ark/error-codes)）。

* 显式指定任务类型，即 `omni_reference_task_type` 为 `reference`、`edit` 或 `extend`：接口在提交任务时提前校验对应任务的特殊参数限制。不符合要求时，接口立即报错，任务不会创建。

<div data-tips="true" data-tips-type="warning" data-tips-is-title="true">注意</div>

<div data-tips="true" data-tips-type="warning">实际处理任务时，模型仍会进一步结合提示词判断任务类型。若实际判定的任务类型和指定的不一致，仍会触发 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_async_error">异步报错</a>（错误码：<a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/error-codes"><code>InvalidParameter.TaskTypeMismatch</code></a>）。建议遵循各任务类型的 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_param_constraints">提示词写法</a>，降低报错概率。</div>
...
* <div data-tips="true" data-tips-type="tip"><code>Seedance 2.5</code> 输出的 1080p 视频和 <code>Seedance 2.0</code> 输出的 4k 视频均采用 10bit 位深与 H.265/HEVC 编码。</div>
  ...
  * <div data-tips="true" data-tips-type="tip">H.265/HEVC 编码在少数播放环境中可能不兼容。如遇问题，建议升级系统、更换设备，或使用 VLC、MPV、QuickTime Player 等播放器查看。播放兼容性说明详见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-0#4k_player">10bit 位深与 H.265/HEVC 编码视频播放兼容性说明</a>。</div>
...
<span aceTableMode="list" aceTableWidth="1,1.5,3,3,2.5"></span>

| 任务类型           |                     | Seedance 2.5                                                                                                                                            | Seedance 2.0 系列                                                                                                          | Seedance 1.0 系列                                                                                          |
| ------------------ | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| 文生视频           |                     | 支持在可选宽高比中自行设置，或由模型根据提示词在可选宽高比中自动选择<br><br>> 支持 `adaptive` 或指定宽高比                                              | 支持在可选宽高比中自行设置，或由模型根据提示词在可选宽高比中自动选择<br><br>> 支持 `adaptive` 或指定宽高比                 | 仅支持在可选宽高比中自行设置<br><br>> 不支持 `adaptive`                                                    |
| 首帧或首尾帧生视频 |                     | **自动保持输出视频宽高比和 ** **`first_frame`** ** 指定的首帧图片一致**<br><br>> 默认且仅支持 `adaptive`                                                | 支持在可选宽高比中自行设置，或由模型根据首帧图在可选宽高比中自动选择<br><br>> 支持 `adaptive` 或指定宽高比                 | 支持在可选宽高比中自行设置，或由模型根据首帧图在可选宽高比中自动选择<br><br>> 支持 `adaptive` 或指定宽高比 |
...
图生视频，选择的宽高比与您上传的图片宽高比不一致时，方舟会对您的图片进行裁剪，裁剪时会居中裁剪，详细规则见 [图片裁剪规则](https://ark.volcengine.com/region:cn-beijing/docs/ark/video-generation-tutorial#f76aafc8)。

<span aceTableMode="list" aceTableWidth="1,1,1,1,1"></span>

| 分辨率                                           | 宽高比 | 宽高像素值 Seedance 2.5 | 宽高像素值 Seedance 2.0 系列 | 宽高像素值 Seedance 1.0 系列 |
| ------------------------------------------------ | ------ | ----------------------- | ---------------------------- | ---------------------------- |
| 480p                                             | `16:9` | 854×480                 | 864×496                      | 864×480                      |
|                                                  | `4:3`  | 752×560                 | 752×560                      | 736×544                      |
|                                                  | `1:1`  | 640×640                 | 640×640                      | 640×640                      |
|                                                  | `3:4`  | 560×752                 | 560×752                      | 544×736                      |
|                                                  | `9:16` | 480×854                 | 496×864                      | 480×864                      |
...
| 1080p<br><br>（Seedance 2.0 fast/mini 暂不支持） | `16:9` | 1920×1080               | 1920×1080                    | 1920×1088                    |
...
<div data-tips="true" data-tips-type="warning">Seedance 2.5 模型在<strong>视频编辑、视频延长、首帧 / 首尾帧生视频任务</strong>（详见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_task_type_intro">任务类型与判定条件</a>）存在特殊限制：</div>
...
当 `duration` 设置为 `-1` 时，实际生成视频的时长可通过 [查询视频生成任务 API](https://ark.volcengine.com/region:cn-beijing/docs/ark/get-video-generation-task-api) 返回的 `duration` 字段获取。视频时长与计费相关，请谨慎设置。

| 模型                         | `duration = -1`取值规则                                                                                                                                                             |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Seedance 2.0 系列            | 模型在 `duration` 的有效取值范围内，自主选择合适的视频长度（整数秒）。                                                                                                              |
...
<div data-tips="true" data-tips-type="tip">通过 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/get-video-generation-task-api">查询视频生成任务 API</a> 接口返回的 <code>duration</code> 值为视频的时长约数（整数秒），与实际的视频时长可能不同。具体返回规则如下：</div>
...
<div data-tips="true" data-tips-type="warning">Seedance 2.5 模型在<strong>视频编辑任务</strong>（详见 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-5#2.5_task_type_intro">任务类型与判定条件</a>）存在特殊限制：</div>
...
* `true`：返回生成视频的尾帧图像，可通过 [查询视频生成任务接口](https://ark.volcengine.com/region:cn-beijing/docs/ark/get-video-generation-task-api) 获取，尾帧图像的格式为 `jpeg`，宽高像素值与生成的视频保持一致，无水印。

* `false`：不返回生成视频的尾帧图像。

使用该参数可实现生成多个连续视频：以上一个生成视频的尾帧作为下一个视频任务的首帧，快速生成多个连续视频，调用示例详见 [教程](https://ark.volcengine.com/region:cn-beijing/docs/ark/video-generation-tutorial#141cf7fa)。

**service_tier** `string` `默认值 default`  |  服务等级

指定处理本次请求的服务等级类型。

* `default`：在线推理模式，RPM 和并发数配额较低（详见 [模型列表](https://ark.volcengine.com/region:cn-beijing/docs/ark/model-list#7571da3f)），适合对推理时效性要求较高的场景。

* `flex`：离线推理模式，TPD 配额更高（详见 [模型列表](https://ark.volcengine.com/region:cn-beijing/docs/ark/model-list#7571da3f)），价格为在线推理的 50%，适合对推理时延要求不高的场景。`Seedance 2.5`、`Seedance 2.0 系列`暂不支持。
...
填写本次生成任务结果的回调通知地址。当视频生成任务有状态变化时，方舟将向此地址推送 POST 请求，请求内容结构与 [查询任务 API](https://ark.volcengine.com/region:cn-beijing/docs/ark/get-video-generation-task-api) 的返回体一致。
...
* `web_search`：联网搜索工具。开启联网搜索后，模型会根据用户的提示词自主判断是否搜索互联网内容（如商品、天气等）。可提升生成视频的时效性，但也会增加一定的时延。[阅读教程](https://ark.volcengine.com/region:cn-beijing/docs/ark/seedance-2-0#c40ed3ef) 获取详细代码示例。

<div data-tips="true" data-tips-type="tip" data-tips-is-title="true">说明</div>

<div data-tips="true" data-tips-type="tip">实际搜索次数可通过 <a href="https://ark.volcengine.com/region:cn-beijing/docs/ark/get-video-generation-task-api">查询视频生成任务 API</a> 返回的 <code>usage.tool_usage.web_search</code> 字段获取，如果为 0 表示未搜索。</div>
...
创建视频生成任务为**异步接口**，获取 ID 后需要通过 [查询视频生成任务 API](https://ark.volcengine.com/region:cn-beijing/docs/ark/get-video-generation-task-api) 来查询任务状态。任务成功后会输出生成视频的 `video_url`。
