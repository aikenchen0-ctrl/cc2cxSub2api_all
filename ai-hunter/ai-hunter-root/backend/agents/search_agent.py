"""SearchAgent — Maps-first discovery with a no-key web fallback."""
...
from tools.search_limits import effective_search_concurrency, uses_paid_maps
...
def _use_paid_maps(settings) -> bool:
    """True when SearchAgent should call Serper Maps instead of free web search."""
    return uses_paid_maps(settings)


def _skipped_free_search(keyword: str, error: str) -> dict:
    """Result row when the free-web circuit already tripped."""
    return {
        "keyword": keyword,
        "results": [],
        "result_count": 0,
...
async def _free_search_keyword(
    keyword: str,
    free_tool: FreeSearchTool,
    semaphore: asyncio.Semaphore,
    *,
    gl: str = "",
    hl: str = "",
    circuit: SearchCircuit | None = None,
) -> dict:
    """Search the public web without API keys and return normalized rows."""
    if circuit is not None and circuit.tripped():
        logger.info("[FreeSearch] skip query=%r — search backend already blocked", keyword)
        return _skipped_free_search(keyword, circuit.error)
    async with semaphore:
        if circuit is not None and circuit.tripped():
            logger.info("[FreeSearch] skip query=%r — search backend already blocked", keyword)
            return _skipped_free_search(keyword, circuit.error)
        try:
            logger.info("[FreeSearch] query=%r gl=%s hl=%s", keyword, gl or "(none)", hl or "(none)")
            rows = await free_tool.search(keyword, gl=gl, hl=hl, allow_encyclopedia=False)
            results = []
            for item in rows:
                link = item.get("link", "")
                if not is_usable_discovery_link(link):
                    continue
                results.append({
                    ...
                    "source": item.get("source") or "free_web",
                })
            logger.info("[FreeSearch] query=%r → %d results", keyword, len(results))
            return {
                "keyword": keyword,
                "results": results,
            ...
            if circuit is not None and isinstance(e, SearchBackendError):
                circuit.trip(str(e))
            logger.warning("[FreeSearch] FAILED query=%r: %s", keyword, e)
            return {
                "keyword": keyword,
                "results": [],
                "result_count": 0,
...
async def search_node(state: HuntState) -> dict:
    """LangGraph node: Maps search when a Serper key exists, else free web search."""
    ...
    concurrency = effective_search_concurrency(settings)
    semaphore = asyncio.Semaphore(concurrency)
    use_maps = _use_paid_maps(settings)
    maps_tool = GoogleMapsSearchTool(settings) if use_maps else None
    free_tool = None if use_maps else FreeSearchTool(settings)
    ...
    if use_maps:
        ...
        route_label = ["google_maps(serper)"]
    else:
        logger.info(
            "[SearchAgent] No SERPER_API_KEY (or SEARCH_BACKEND=free) — using free web fallback (rate-limited)"
        )
        search_tasks = [
            _free_search_keyword(kw, free_tool, semaphore, gl=gl, hl=hl, circuit=circuit)
            for kw in keywords
        ]
        route_label = ["free_web(duckduckgo)"]
    logger.info(
        "[SearchAgent] Tool routing: regions=%s → %s",
        target_regions,
        route_label,
    )
    logger.info("[SearchAgent] Launching %d search tasks (concurrency=%d)", len(search_tasks), concurrency)
    ...
    for r in raw_results:
        ...
        if r.get("error"):
            keyword_stats[base_kw]["error"] = str(r["error"])
    ...
    for tool in (maps_tool, free_tool):
        if tool is None:
            continue
        try:
            await tool.close()
