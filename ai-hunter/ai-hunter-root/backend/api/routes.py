...
def _mark_hunt_stage(hunt_id: str, stage: str) -> None:
    """Persist the node that is about to run so a mid-node crash is not blamed on the previous stage."""
    hunt = _hunts.get(hunt_id)
    if hunt is None:
        return
    if hunt.get("current_stage") == stage:
        return
    hunt["current_stage"] = stage
    ...
    _broadcast(hunt_id, "stage_change", {
        "stage": stage,
        "hunt_round": hunt.get("hunt_round", 1),
        "leads_count": hunt.get("leads_count", 0),
    })


def _failed_stage_from_hunt(hunt_id: str, accumulated: dict) -> str:
    """Prefer the node that started over the last completed node."""
    hunt = _hunts.get(hunt_id) or {}
    return str(
        hunt.get("current_stage")
        or accumulated.get("current_stage")
        or ""
    )
...
async def _run_hunt(hunt_id: str, request: HuntRequest) -> None:
    ...
    try:
        ...
        graph = build_graph(
            ...
            on_node_start=lambda stage: _mark_hunt_stage(hunt_id, stage),
        )
...
def _slim_state(prior_result: dict, request: ResumeRequest) -> dict:
    ...
    if keyword_search_stats:
        ...
        resume_feedback: dict | None = {
            ...
            "region_distribution": {},
            "search_backend_error": collect_search_backend_error(keyword_search_stats),
        }
    else:
...
async def _run_resume_hunt(hunt_id: str, request: ResumeRequest, prior_result: dict) -> None:
    ...
    try:
        ...
        remove_tracker(hunt_id)
        accumulated["cost_summary"] = cost_summary
        failed_stage = _failed_stage_from_hunt(hunt_id, accumulated)
        if failed_stage:
            accumulated["current_stage"] = failed_stage
        _hunts[hunt_id].update({
            "status": "cancelled",
            "error": str(e),
            "completed_at": now_iso(),
            "cost_summary": cost_summary,
            "result": accumulated,
            "current_stage": failed_stage or _hunts[hunt_id].get("current_stage"),
            "leads_count": _unique_leads_count(accumulated.get("leads", [])),
            "hunt_round": accumulated.get("hunt_round", 0),
        })
        save_hunt(hunt_id, _hunts[hunt_id])
...
def _hunt_leads_csv(hunt_id: str | None = None) -> str:
    if hunt_id:
        if hunt_id not in _hunts:
            raise HTTPException(status_code=404, detail="Hunt not found")
    ...
    for hunt in hunts:
        result = hunt.get("result") or {}
        leads.extend(_dedupe_leads(result.get("leads", []) or []))
    return leads_to_csv(leads)


def _hunt_operator_report(hunt_id: str) -> str:
    if hunt_id not in _hunts:
        raise HTTPException(status_code=404, detail="Hunt not found")
    hunt = _hunts[hunt_id]
    ...
    return render_hunt_report(hunt_id, result)


@router.get("/hunts/{hunt_id}/export-report.txt", dependencies=[Depends(require_api_access)])
async def export_hunt_report(hunt_id: str):
    """Export operator report: insight, keywords, raw search counts, keyword effectiveness."""
    body = _hunt_operator_report(hunt_id)
    filename = f"hunt-{hunt_id[:8]}-report.txt"
    return Response(
        content=body,
...
@router.get("/hunts/{hunt_id}/export.csv", dependencies=[Depends(require_api_access)])
async def export_hunt_csv(hunt_id: str):
    """Export hunt leads as Instantly/Smartlead-aligned CSV (includes `reason`)."""
    csv_body = _hunt_leads_csv(hunt_id)
    filename = f"hunt-{hunt_id[:8]}-leads.csv"
    return Response(
        content=csv_body,
...
@router.get("/export.csv", dependencies=[Depends(require_api_access)])
async def export_csv(hunt_id: str | None = None):
    """Export leads as Instantly/Smartlead-aligned CSV. Optional `hunt_id` filter."""
    csv_body = _hunt_leads_csv(hunt_id)
    filename = f"hunt-{hunt_id[:8]}-leads.csv" if hunt_id else "leads.csv"
    return Response(
        content=csv_body,
        media_type="text/csv; charset=utf-8",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
