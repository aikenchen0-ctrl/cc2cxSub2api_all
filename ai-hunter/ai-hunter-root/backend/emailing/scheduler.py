...
async def run_scheduler_once(
    ...
    *,
    ...
    for job in jobs:
        ...
        settings = get_settings()
        recipient = str(sequence.get("lead_email", "") or "")
        ...
        if not recipient_verdict.ok:
            store.mark_message_failed(
                str(job["id"]),
                failure_reason=recipient_verdict.reason,
                updated_at=current,
            )
            if "unsubscribed" in recipient_verdict.reason or "suppressed" in recipient_verdict.reason:
                store.cancel_future_pending_messages(str(sequence["id"]), updated_at=current)
                store.update_sequence_status(
                    str(sequence["id"]),
                    status="stopped",
        ...
        if sender is send_email:
            sent_today = store.count_messages_since(
                "sent",
            ...
            window = screen_send_window_from_settings(
                ...
                last_sent_at=str(sequence.get("last_sent_at", "") or ""),
                now=datetime.fromisoformat(current.replace("Z", "+00:00")) if "T" in current else None,
            )
            if not window.ok:
                skipped += 1
                continue
