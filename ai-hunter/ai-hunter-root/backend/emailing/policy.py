...
def screen_email_targets(
    ...
    *,
    ...
    """Ranked targets that pass outbound recipient screens."""
    sendable: list[dict[str, str]] = []
    for target in expand_email_targets(lead):
        email = str(target.get("target_email") or "")
        if not email:
            continue
        verdict = screen_recipient(
            email,
            settings=settings,
            sender_email=sender_email,
            user_supplied=bool(lead.get("email_source") in {"manual", "imported"}),
            store=store,
        )
        if verdict.ok:
