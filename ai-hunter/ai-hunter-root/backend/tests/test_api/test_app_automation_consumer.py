...
async def test_embedded_consumer_claims_and_completes_job(monkeypatch, tmp_path):
    ...
    settings = type(
        ...
        (),
        {
            ...
            "automation_consumer_max_attempts": 3,
...
@pytest.mark.asyncio
async def test_embedded_consumer_marks_401_as_failed(monkeypatch, tmp_path):
    queue_path = str(tmp_path / "queue.db")
    queue = HuntJobQueue(queue_path)
    queue.init_db()
    job_id = queue.enqueue(
    ...
    monkeypatch.setattr(
        "api.app._run_embedded_consumer_job",
        AsyncMock(side_effect=RuntimeError("AuthenticationError: Error code: 401 - unauthorized_error")),
    )
