...
class TestInsightNode:
    ...
    async def test_returns_insight_and_stage(self):
        ...
        with patch("agents.insight_agent.react_loop", return_value=FAKE_INSIGHT_JSON), \
             ...
             patch("agents.insight_agent.open_web_search") as MockGoogle:
    ...
    @pytest.mark.asyncio
    async def test_site_brief_from_scan_injected_into_react(self):
        """A successful site scan is injected into the ReAct prompt and returned."""
        state = _base_state()
        captured_kwargs = {}
        ...
        brief = {
            ...
            "domain": "solartech.de",
            "provider": "fetch",
            "pages": [{"url": "https://solartech.de", "title": "Home", "markdown": "CE certified inverters"}],
            "brief_markdown": "### Home\nSource: https://solartech.de\nCE certified inverters",
        }

        with patch("agents.insight_agent.react_loop", side_effect=capture_react_loop), \
             patch("agents.insight_agent.scan_site", AsyncMock(return_value=brief)), \
             patch("agents.insight_agent.JinaReaderTool") as MockJina, \
             patch("agents.insight_agent.open_web_search") as MockGoogle:
        ...
        assert "Pre-crawled Site Brief" in captured_kwargs["user_prompt"]
        assert "CE certified inverters" in captured_kwargs["user_prompt"]
        assert result["site_brief"]["provider"] == "fetch"
        assert result["insight"]["company_name"] == "SolarTech GmbH"

    @pytest.mark.asyncio
    async def test_site_scan_failure_does_not_abort_hunt(self):
        """scan_site exceptions are non-fatal — Insight still runs."""
        state = _base_state()

        with patch("agents.insight_agent.react_loop", return_value=FAKE_INSIGHT_JSON), \
             patch("agents.insight_agent.scan_site", AsyncMock(side_effect=RuntimeError("crawl down"))), \
             patch("agents.insight_agent.JinaReaderTool") as MockJina, \
             patch("agents.insight_agent.open_web_search") as MockGoogle:
        ...
        assert result["insight"]["company_name"] == "SolarTech GmbH"
        assert "site_brief" not in result
...
class TestBuildUserPrompt:
    ...
    def test_site_brief_injected_when_present(self):
        prompt = _build_user_prompt(
            ...
            site_brief={
                "skipped": False,
                "provider": "fetch",
                "pages": [{"url": "https://acme.com", "title": "Acme", "markdown": "We sell inverters."}],
                "brief_markdown": "### Acme\nSource: https://acme.com\nWe sell inverters.",
            },
        )
        assert "Pre-crawled Site Brief" in prompt
        assert "We sell inverters." in prompt
        assert "do not re-scrape" in prompt

    def test_skipped_site_brief_not_injected(self):
        prompt = _build_user_prompt(
            website_url="https://acme.com",
            product_keywords=[],
            target_regions=[],
            parsed_docs=[],
            site_brief={"skipped": True, "reason": "disabled", "brief_markdown": "", "pages": []},
        )
        assert "Pre-crawled Site Brief" not in prompt
