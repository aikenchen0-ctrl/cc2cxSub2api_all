...
function statusLabel(status: string, t: (key: string) => string) {
  ...
  if (status === "running") return t("status.executing");
...
function templateSeedLabel(status: string, t: (key: string) => string) {
  if (status === "ready") return t("status.ready");
  if (status === "preparing") return t("status.preparing");
  if (status === "failed") return t("status.prepareFailed");
  return t("status.notReady");
}

function streamLabel(state: string, t: (key: string) => string) {
  if (state === "connected") return t("stream.connected");
  if (state === "connecting") return t("stream.connecting");
  if (state === "fallback") return t("stream.fallback");
  return t("stream.waiting");
}
...
export function AutomationJobPage() {
  ...
  useEffect(() => {
    ...
    source.onopen = () => {
      if (!ended) setStreamState("connected");
    };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline">{streamLabel(streamState, t)}</Badge>
          <Badge variant={statusVariant(job.status)}>{statusLabel(job.status, t)}</Badge>
          ...
          {(job.status === "failed" || job.status === "completed") && (
            ...
            >
              {retryMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : t("job.retry")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          ...
          <CardContent className="text-2xl font-semibold">{statusLabel(job.status, t)}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
        ...
        <Card>
          ...
          <CardContent className="text-2xl font-semibold">
            {templateSeedLabel(job.template_seed_status, t)}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
        ...
        <Card>
          ...
          <CardContent className="text-2xl font-semibold">
            {templateSeedWorker?.running ? t("common.online") : t("common.offline")}
          </CardContent>
        </Card>
      </div>
