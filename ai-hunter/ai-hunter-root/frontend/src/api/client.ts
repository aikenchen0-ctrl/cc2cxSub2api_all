...
export interface LicensedFinderStatus {
  enabled: boolean;
  has_api_key: boolean;
  base_url: string;
  in_graph: boolean;
...
export interface LicensedFinderLead {
  company_name?: string;
  company?: string;
  website?: string;
  emails?: string[];
...
export interface LicensedFinderResult {
  request_id?: string;
  credits_consumed?: number;
  credits_left?: number | null;
  leads_found?: number;
  leads: LicensedFinderLead[];
  raw_status?: string;
}

export interface LicensedFinderSearchRequest {
...
export interface LicensedFinderEnrichRequest {
  leads: Record<string, unknown>[];
  enrich_emails?: boolean;
  enrich_phones?: boolean;
...
function formatApiError(detail: unknown, fallback: string): string {
  ...
  if (detail && typeof detail === "object") {
    const rec = detail as { message?: unknown; type?: unknown; detail?: unknown };
    if (typeof rec.message === "string" && rec.message.trim()) {
      return typeof rec.type === "string" && rec.type ? `${rec.type}: ${rec.message}` : rec.message;
    }
...
async function licensedFinderRequest<T>(path: string, options?: RequestInit): Promise<T> {
  const controller = new AbortController();
  const timeoutMs = 320000;
...
export const api = {
  ...
  listSuppressions: (limit = 200) =>
    request<SuppressionListResponse>(`/suppressions?limit=${limit}`),

  addSuppression: (email: string, reason = "operator") =>
    request<SuppressionAddResponse>("/suppressions", {
      method: "POST",
      body: JSON.stringify({ email, reason }),
    }),

  getLicensedFinderStatus: () =>
    request<LicensedFinderStatus>("/licensed-finder/status"),

  searchLicensedFinder: (data: LicensedFinderSearchRequest) =>
    licensedFinderRequest<LicensedFinderResult>("/licensed-finder/search", {
      method: "POST",
      body: JSON.stringify(data),
    }),

  enrichLicensedFinder: (data: LicensedFinderEnrichRequest) =>
    licensedFinderRequest<LicensedFinderResult>("/licensed-finder/enrich", {
      method: "POST",
      body: JSON.stringify(data),
    }),

  exportHuntCsv: async (huntId: string): Promise<{ blob: Blob; filename: string; text: string }> => {
    const controller = new AbortController();
    ...
    const filename = match?.[1] || `hunt-${huntId.slice(0, 8)}-leads.csv`;
    const blob = new Blob([text], { type: "text/csv;charset=utf-8" });
    return { blob, filename, text };
  },

  exportHuntReport: async (huntId: string): Promise<{ blob: Blob; filename: string; text: string }> => {
    const controller = new AbortController();
    const timeoutMs = 60000;
    const timeout = window.setTimeout(() => controller.abort(), timeoutMs);
    let res: Response;
    ...
    const disposition = res.headers.get("Content-Disposition") || "";
    const match = disposition.match(/filename="?([^"]+)"?/i);
    const filename = match?.[1] || `hunt-${huntId.slice(0, 8)}-report.txt`;
