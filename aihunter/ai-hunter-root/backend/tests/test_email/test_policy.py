from emailing.policy import choose_email_target, expand_email_targets, screen_email_targets
...
def test_screen_email_targets_drops_role_and_freemail():
    lead = {
        "decision_makers": [
            {"name": "Jane Doe", "title": "Purchasing Manager", "email": "jane.doe@acme.com"},
        ],
        "emails": ["info@acme.com", "jane.doe@gmail.com"],
    }
    settings = type("S", (), {"email_block_freemail": True, "email_allow_generic_company_email": False})()
    targets = screen_email_targets(lead, settings=settings, sender_email="sales@example.com")
    assert [item["target_email"] for item in targets] == ["jane.doe@acme.com"]


def test_screen_email_targets_drops_suppressed():
    lead = {
        "decision_makers": [
            {"name": "Jane Doe", "title": "Purchasing Manager", "email": "jane.doe@acme.com"},
            {"name": "Pat Lee", "title": "Owner", "email": "pat.lee@acme.com"},
        ],
        "emails": [],
    }
    settings = type("S", (), {"email_block_freemail": True, "email_allow_generic_company_email": False})()
    store = type("Store", (), {"is_suppressed": staticmethod(lambda email: email == "jane.doe@acme.com")})()
    targets = screen_email_targets(lead, settings=settings, sender_email="sales@example.com", store=store)
    assert [item["target_email"] for item in targets] == ["pat.lee@acme.com"]
