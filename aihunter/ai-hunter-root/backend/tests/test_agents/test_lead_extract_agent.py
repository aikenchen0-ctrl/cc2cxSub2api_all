...
def _node_settings(**overrides):
    """Settings for lead_extract_node tests.

    MagicMock truthiness would turn OSINT/social on and skip MX via a
    truthy email_verify_min_fit_score. Pin those flags instead.
    """
    settings = MagicMock()
...
class TestCandidateBudget:
    ...
    @pytest.mark.asyncio
    async def test_scrape_page_falls_back_to_direct_fetch(self):
        from agents.lead_extract_agent import _build_react_tools

        jina = AsyncMock()
        jina.read = AsyncMock(side_effect=Exception("jina 403"))
        tools = _build_react_tools(jina, AsyncMock(), AsyncMock(), {})
        scrape_tool = next(t for t in tools if t.name == "scrape_page")

        with patch(
            "agents.lead_extract_agent.fetch_page_text",
            new=AsyncMock(return_value="Acme Solar is a German inverter distributor with EU stock."),
        ) as fetch_mock:
            result = await scrape_tool.fn(url="https://acme.test/")

        parsed = json.loads(result)
        assert "inverter distributor" in parsed["content"]
...
class TestLeadExtractNode:
    ...
    async def test_extracts_leads(self):
        ...
        with patch("agents.lead_extract_agent.JinaReaderTool") as MockJina, \
             ...
             patch("agents.lead_extract_agent.open_web_search") as MockGoogle, \
            ...
            mock_settings.return_value = _node_settings()
...
class TestEmailVerification:
    ...
    async def test_undeliverable_emails_removed(self):
        ...
        with patch("agents.lead_extract_agent.JinaReaderTool") as MockJina, \
            ...
            mock_settings.return_value = _node_settings(email_verify_min_fit_score=0.0)
            MockJina.return_value = AsyncMock(close=AsyncMock())
            MockLLM.return_value = AsyncMock(close=AsyncMock())
            MockGoogle.return_value = AsyncMock(close=AsyncMock())
