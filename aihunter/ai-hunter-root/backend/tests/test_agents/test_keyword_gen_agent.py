...
class TestKeywordGenNode:
    ...
    async def test_respects_max_keywords_per_round(self):
        ...
        with patch("agents.keyword_gen_agent.LLMTool") as MockLLM, \
            ...
            mock_settings.return_value.serper_api_key = "k"
    ...
    @pytest.mark.asyncio
    async def test_caps_keywords_without_serper(self):
        state = _base_state()
        many_keywords = [f"keyword_{i}" for i in range(20)]

        with patch("agents.keyword_gen_agent.LLMTool") as MockLLM, \
             patch("agents.keyword_gen_agent.get_settings") as mock_settings:
            mock_settings.return_value.default_keywords_per_round = 8
            mock_settings.return_value.search_backend = "auto"
...
class TestBuildPrompt:
    ...
    def test_includes_search_backend_error(self):
        feedback = {
            "round": 1,
            "total_leads": 0,
            "target": 20,
        ...
        prompt = _build_prompt(_base_state(round_feedback=feedback))
        assert "Search backend blocked" in prompt
        assert "ddg_html: 502 Bad Gateway" in prompt
        assert "not a keyword miss" in prompt
