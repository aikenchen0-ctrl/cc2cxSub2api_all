...
class TestEvaluateProgress:
    ...
    def test_feedback_includes_search_backend_error(self):
        stats = {
            "kw1": {"result_count": 0, "leads_found": 0, "error": "ddg_html: 502 Bad Gateway"},
            "kw2": {"result_count": 0, "leads_found": 0, "error": "ddg_html: 502 Bad Gateway"},
        }
        result = evaluate_progress(self._make_state(
            keyword_search_stats=stats,
            keywords=["kw1", "kw2"],
        ))
        assert result["round_feedback"]["search_backend_error"] == "ddg_html: 502 Bad Gateway"

    def test_feedback_ignores_historical_search_errors(self):
        stats = {
            "old": {"result_count": 0, "leads_found": 0, "error": "ddg_html: 502 Bad Gateway"},
            "new": {"result_count": 8, "leads_found": 2},
        }
        result = evaluate_progress(self._make_state(
            keyword_search_stats=stats,
            keywords=["new"],
        ))
        assert result["round_feedback"]["search_backend_error"] == ""
...
class TestShouldContinueHunting:
    ...
    def test_finish_when_zero_leads_after_round_1(self):
        state = self._make_state(
            leads=[],
            hunt_round=2,
            round_feedback=self._feedback(round_num=1, new_leads=0),
        )
        assert should_continue_hunting(state) == "finish"
    ...
    def test_finish_when_search_blocked_even_with_prior_leads(self):
        """Resume hunts already have old leads; blocked search must still stop."""
        leads = [{"company": f"C{i}"} for i in range(10)]
        state = self._make_state(
            leads=leads,
            hunt_round=2,
            round_feedback={
    ...
    def test_continue_when_search_blocked_but_round_found_leads(self):
        leads = [{"company": f"C{i}"} for i in range(8)]
        state = self._make_state(
            leads=leads,
            hunt_round=2,
            round_feedback={
                "round": 1,
                "new_leads_this_round": 8,
                "search_backend_error": "ddg_html: 502 Bad Gateway",
            },
        )
        assert should_continue_hunting(state) == "continue"
