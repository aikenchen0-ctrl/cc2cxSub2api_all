...
import com.artisanlab.auth.Sub2ApiRelayKeys;
import com.artisanlab.auth.Sub2ApiSsoTickets;
import com.artisanlab.auth.UserIdentityEntity;
import com.artisanlab.auth.UserIdentityMapper;
...
public class UserApiConfigService {
    ...
    private static final String ONLY_CHAT_MODEL = "gpt-5.5";
    private static final Set<String> CHAT_MODELS = Set.of("gpt-5.5", "gpt-5.4-mini");
    ...
    public UserApiConfigDtos.ResolvedConfig getRequiredConfig(UUID userId) {
        UserApiConfigEntity entity = mapper.findByUserId(userId);
        if (entity != null) {
            return overlayRelayKey(userId, toResolvedConfig(entity));
        }
        UserApiConfigDtos.ResolvedConfig serverDefault = getServerDefaultResolvedConfig();
        if (serverDefault != null) {
            return overlayRelayKey(userId, serverDefault);
        }
    ...
    private UserApiConfigDtos.ResolvedConfig getServerDefaultResolvedConfig() {
        ...
        String chatModel = normalizeModel(
                ...
                CHAT_MODELS,
                "默认对话模型仅支持 gpt-5.5 或 gpt-5.4-mini"
        );
    ...
    private UserApiConfigDtos.ResolvedConfig overlayRelayKey(UUID userId, UserApiConfigDtos.ResolvedConfig config) {
        String bound = Sub2ApiRelayKeys.get(userId);
        if ((bound == null || bound.isBlank()) && identityMapper != null && userId != null) {
            ...
            if (identity != null && identity.getSubject() != null && !identity.getSubject().isBlank()) {
                bound = identity.getSubject().trim();
                Sub2ApiRelayKeys.put(userId, bound);
            }
        }
        if (config == null || bound == null || bound.isBlank()) {
            return config;
        }
        return new UserApiConfigDtos.ResolvedConfig(
                config.baseUrl(),
