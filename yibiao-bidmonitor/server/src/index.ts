...
const monitorClient = new BidMonitorClient({
  baseUrl: process.env.BID_MONITOR_URL || 'http://127.0.0.1:8080',
  serviceToken: process.env.BID_MONITOR_SERVICE_TOKEN || '',
...
await app.register(
  async (protectedApp) => {
    ...
    // 模块门禁：招标监控。普通用户需 modules 含 bid-monitor；Python 服务只由 Fastify 内部调用。
    await protectedApp.register(async (monitorApp) => {
      monitorApp.addHook('onRequest', createRequireModule(prisma, 'bid-monitor'));
      await monitorApp.register(monitorRoutes, { prefix: '/api' });
    });
