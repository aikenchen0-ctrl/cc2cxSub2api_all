...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    publicHttp
      .post<unknown>('/auth/sso/exchange', { handoff })
      .then(({ data }) => {
        if (!active || !isAuthenticatedResponse(data)) return;
        completeLogin(data.token, data.user);
        window.history.replaceState({}, document.title, safeRedirect);
      })
      .catch(() => {
        if (active) window.history.replaceState({}, document.title, '/');
      });
