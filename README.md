...
- [卫星应用接入公共层](docs/卫星应用接入公共层.md)（给 AI / 开发者的对接手册）
- [卫星公开模型](docs/卫星公开模型.md)（文本 `gpt-5.5` / 图片 `gpt-image-2` / 视频 `grok-imagine-video-1.5`）
- [本地链接.txt](本地链接.txt)（`LINK` 与 `{项目}_link`）
- [AGENTS.md](AGENTS.md)

根目录中的其他中文 Markdown 记录模型适配、数据导入和阶段性方案；主项目专题文档在 `sub2api/docs/`。涉及支付、合规、插件和安全配置时，请优先阅读对应专题文档。
