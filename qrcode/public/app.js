...
const SUB2API_REQUEST_HEADERS = (() => {
  const params = new URLSearchParams(window.location.search);
  const apiKey = (params.get("apiKey") || params.get("apikey") || "").trim();
  const baseUrl = (params.get("baseUrl") || params.get("baseurl") || "").trim();
  return apiKey && baseUrl
    ? { "X-Sub2API-API-Key": apiKey, "X-Sub2API-Base-URL": baseUrl }
    : {};
})();
const originalFetch = (window.fetch || globalThis.fetch).bind(globalThis);
window.fetch = (input, options = {}) => {
  const target = typeof input === "string" ? input : input?.url || "";
  if (!target.includes(API_BASE_PATH) || !Object.keys(SUB2API_REQUEST_HEADERS).length) {
    return originalFetch(input, options);
  }
  const headers = new Headers(options.headers || {});
  Object.entries(SUB2API_REQUEST_HEADERS).forEach(([name, value]) => headers.set(name, value));
