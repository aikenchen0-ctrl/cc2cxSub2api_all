...
import { analyzeAssetImage, create3dGeneration, get3dApiHealth, get3dServerLogs, getAvailableGenerationModes, getGenerationRequestOptions, getProviderLabel, getProviderPlan, resolveGenerationMode, uploadLocal3dModel, waitFor3dModel } from './services/modelApi.js'
...
function isSketchGenerationMode(mode) {
  return mode === HUNYUAN_SKETCH_MODE
}

function formatSketchGenerationError(error) {
  const message = error instanceof Error ? error.message : String(error || '')
  if (/quota|credit|balance|积分|余额|insufficient|2010|credit.?limit/i.test(message)) {
    return '积分不足，请去腾讯云控制台充值，Key 不用换。'
  }
  if (/\b(?:400|422)\b|sketch|line|photo|照片|线稿|image|图片|prompt|输入|invalid/i.test(message)) {
    return '需要线稿，请上传清晰的草图后重试。'
  }
  return message || '混元草图模式生成失败。'
}
...
function App() {
  ...
  const [pendingSketchUpload, setPendingSketchUpload] = useState(null)
  ...
  const resumedTasksRef = useRef(new Set())
  ...
  const selectedGenerationMode = GENERATION_MODE_IDS.has(settings.generationMode) ? settings.generationMode : 'hunyuan'
  const availableGenerationModes = useMemo(
    () => getAvailableGenerationModes(apiHealth, selectedGenerationMode),
    [apiHealth, selectedGenerationMode],
  )
  const activeGenerationMode = useMemo(
    () => resolveGenerationMode(selectedGenerationMode, apiHealth),
    [apiHealth, selectedGenerationMode],
  )

  useEffect(() => {
    return () => {
      if (pendingSketchUpload?.previewUrl) URL.revokeObjectURL(pendingSketchUpload.previewUrl)
      if (pendingImageUpload?.previewUrl) URL.revokeObjectURL(pendingImageUpload.previewUrl)
    }
  }, [pendingImageUpload, pendingSketchUpload])
  ...
  useEffect(() => {
    customCells.forEach((cell) => {
      ...
      const taskKey = `${cell.id}:${generation.taskId || ''}`
      if (!cell.custom || !generation.taskId || generation.modelUrl) return
      if (['failed', 'success', 'local'].includes(status)) return
      if (resumedTasksRef.current.has(taskKey)) return

      resumedTasksRef.current.add(taskKey)
      const provider = generation.provider || 'hunyuan'
      const sketchMode = isSketchGenerationMode(generation.requestedProvider)
      const label = sketchMode ? '混元草图' : getProviderLabel(provider)
      setToast(`${label} 正在恢复已提交任务`)
      waitFor3dModel(generation.taskId, provider, (pollStatus) => {
        updateCustomCell(cell.id, (current) => ({
          generation: {
            ...current.generation,
            provider,
            status: pollStatus.status || 'processing',
            progress: pollStatus.progress ?? current.generation?.progress ?? null,
            taskId: generation.taskId,
            message: sketchMode ? '正在用混元草图模式生成，请不要刷新。' : pollStatus.progress ? `${label} progress ${pollStatus.progress}%` : `${label} status: ${pollStatus.status || 'processing'}`,
          },
        }))
      }).then((finalStatus) => {
        updateCustomCell(cell.id, (current) => ({
        ...
        setToast(`${cell.name} ${label} 3D model ready`)
      }).catch((error) => {
        updateCustomCell(cell.id, (current) => ({
          generation: {
            ...current.generation,
            status: 'failed',
            progress: null,
            message: sketchMode ? formatSketchGenerationError(error) : error instanceof Error ? error.message : '3D generation failed.',
          },
        }))
        setToast(sketchMode ? formatSketchGenerationError(error) : error instanceof Error ? error.message : 'Image-to-3D generation failed')
      })
    })
  }, [customCells])
  ...
  async function generateCustomCellModel(customCell, imageUrl, fileName, requestedProvider = activeGenerationMode, generationPrompt = sketchPrompt) {
    const providers = getProviderPlan(requestedProvider)
    const requestOptions = getGenerationRequestOptions(requestedProvider, generationPrompt)
    const sketchMode = isSketchGenerationMode(requestedProvider)
    const errors = []
      ...
      try {
        updateCustomCell(customCell.id, (cell) => ({
          generation: {
            ...
            modelUrl: '',
            rawModelUrl: '',
            message: sketchMode ? '正在用混元草图模式生成，请不要刷新。' : `Sending image to ${label}.`,
          },
        }))
        updateGenerationHistory(historyId, {
          status: 'uploading',
        ...
        const task = await create3dGeneration({
          ...
          fileName,
          prompt: requestOptions.provider === 'hunyuan' ? requestOptions.prompt : getGenerationPrompt(customCell),
          model: requestOptions.provider === 'hunyuan' ? requestOptions.model : undefined,
          generateType: requestOptions.provider === 'hunyuan' ? requestOptions.generateType : undefined,
        ...
        updateCustomCell(customCell.id, (cell) => ({
          generation: {
            ...
            progress: 0,
            taskId: task.taskId,
            message: sketchMode ? '正在用混元草图模式生成，请不要刷新。' : `${label} is generating the GLB model.`,
          },
        }))
        updateGenerationHistory(historyId, {
          status: 'processing',
        ...
        const finalStatus = await waitFor3dModel(task.taskId, provider, (status) => {
          updateCustomCell(customCell.id, (cell) => ({
            generation: {
              ...
              taskId: task.taskId,
              message: sketchMode ? '正在用混元草图模式生成，请不要刷新。' : status.progress ? `${label} progress ${status.progress}%` : `${label} status: ${status.status || 'processing'}`,
            },
          }))
          updateGenerationHistory(historyId, {
            status: status.status || 'processing',
        ...
        const message = sketchMode ? formatSketchGenerationError(error) : error instanceof Error ? error.message : `${label} generation failed.`
  ...
  async function handleRetryGeneration(cellId) {
    ...
    const retryMode = cell.generation?.requestedProvider || (activeGenerationMode === 'local' ? 'hunyuan' : activeGenerationMode)
    ...
    try {
      ...
      updateCustomCell(cell.id, (current) => ({
        generation: {
          ...current.generation,
          requestedProvider: retryMode,
          status: 'failed',
          progress: null,
          modelUrl: '',
          rawModelUrl: '',
          message: isSketchGenerationMode(retryMode) ? formatSketchGenerationError(error) : error instanceof Error ? error.message : '3D generation failed.',
        },
      }))
      const retryMessage = isSketchGenerationMode(retryMode) ? formatSketchGenerationError(error) : error instanceof Error ? error.message : 'Image-to-3D generation failed'
      setToast(retryMessage)
    }
  }

  function handleUploadImage(file) {
    if (isSketchGenerationMode(activeGenerationMode)) {
      if (!file.type?.startsWith('image/')) {
        setToast('草图模式只接受图片线稿。')
        return Promise.resolve()
      }
      const previewUrl = URL.createObjectURL(file)
      setPendingSketchUpload({ file, name: file.name, previewUrl })
      setToast('请上传线稿，并填写提示词。')
      return Promise.resolve()
    }

    if (isLocalModelFile(file)) {
      return handleUploadLocalModel(file)
    }

    const previewUrl = URL.createObjectURL(file)
    setPendingImageUpload({ file, name: file.name, previewUrl, mode: activeGenerationMode })
    setToast('请确认图片后开始生成。')
    return Promise.resolve()
  }

  async function confirmSketchUpload() {
    if (!pendingSketchUpload?.file) return
    const { file } = pendingSketchUpload
    setPendingSketchUpload(null)
    await processImageUpload(file, HUNYUAN_SKETCH_MODE, sketchPrompt)
  }
  ...
  async function confirmImageUpload() {
    if (!pendingImageUpload?.file) return
    const { file, mode } = pendingImageUpload
    setPendingImageUpload(null)
    await processImageUpload(file, mode, '')
  }
      ...
      if (customCell) {
        updateCustomCell(customCell.id, (cell) => ({
          generation: {
            ...
            status: 'failed',
            progress: null,
            message: isSketchGenerationMode(resolvedMode) ? formatSketchGenerationError(error) : error instanceof Error ? error.message : '3D generation failed.',
          },
        }))
      }
      const uploadMessage = isSketchGenerationMode(resolvedMode) ? formatSketchGenerationError(error) : error instanceof Error ? error.message : 'Image-to-3D generation failed'
      setToast(uploadMessage)
    }
  }
    ...
    <main className={settings.compactUi ? 'studio-shell compact-ui' : 'studio-shell'}>
      <motion.div className={demoMode ? 'studio-window workbench-v2 demo-mode' : 'studio-window workbench-v2'} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38 }}>
        ...
        <div className="studio-workbench-v2">
          ...
          {inspectorOpen && (
            <>
              <button type="button" className="inspector-scrim" aria-label={t(settings.language, 'closeInspector')} onClick={() => setInspectorOpen(false)} />
          ...
          <div className="command-zone">
            <BottomDeck
              ...
              generationModes={availableGenerationModes}
              onGenerationModeChange={(generationMode) => setSettings((current) => normalizeSettings({ ...current, generationMode, generationProvider: generationMode }))}
              ...
              pendingSketchUpload={pendingSketchUpload}
              pendingImageUpload={pendingImageUpload}
              onSketchPromptChange={setSketchPrompt}
              onConfirmSketchUpload={confirmSketchUpload}
              onCancelSketchUpload={cancelSketchUpload}
              onConfirmImageUpload={confirmImageUpload}
