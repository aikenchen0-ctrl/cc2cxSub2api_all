...
function resolveRequestAiConfig(request) {
  const apiKey = String(request.get("X-Sub2API-API-Key") || "").trim();
  const baseUrl = String(request.get("X-Sub2API-Base-URL") || "").trim().replace(/\/+$/, "");
  ...
  return { apiKey, apiBaseUrl: baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1` };
}
...
function createGenerateJobScheduler(generateJobs, generateConcurrency, { includeDebugPipeline = true } = {}) {
  ...
  return function createGenerateJob(runGenerate) {
  ...
  app.post("/api/generate", upload.fields([
    ...
    try {
      ...
      const runGenerate = () => withGenerateTimeout(generate({
          ...
          positivePrompt: request.body?.positivePrompt,
          negativePrompt: request.body?.negativePrompt,
          ...resolveRequestAiConfig(request)
        }));

      if (shouldRunGenerateAsync(request.body)) {
        const job = createGenerateJob(runGenerate);
